...
static void nr_ue_mac_config_mbs_sib20(NR_UE_MAC_INST_t *mac, NR_SIB20_r17_t *sib20)
{
  if (!sib20)
    return;

  asn1cFreeStruc(asn_DEF_NR_SIB20_r17, mac->mbs_sib20);
  asn_copy(&asn_DEF_NR_SIB20_r17, (void **)&mac->mbs_sib20, sib20);

  const NR_MCCH_Config_r17_t *mcch = &sib20->mcch_Config_r17;
  const long window_duration = mcch->mcch_WindowDuration_r17 ? *mcch->mcch_WindowDuration_r17 : -1;
  LOG_I(NR_MAC,
        "[UE %d] configured NR MBS SIB20-r17: mcch-repetition present %d, windowStartSlot %ld, windowDuration enum %ld, modificationPeriod enum %ld, cfrConfig %s\n",
        mac->ue_id,
        mcch->mcch_RepetitionPeriodAndOffset_r17.present,
        mcch->mcch_WindowStartSlot_r17,
        window_duration,
        mcch->mcch_ModificationPeriod_r17,
        sib20->cfr_ConfigMCCH_MTCH_r17 ? "present" : "absent");

  if (!mac->mbs.enabled) {
    mac->mbs.enabled = true;
    mac->mbs.g_rnti = 0xfe01;
    LOG_I(NR_MAC,
          "[UE %d] enabled NR MBS MTCH monitoring with OCUDU default G-RNTI 0x%04x; override with OAI_NRUE_MBS_G_RNTI\n",
          mac->ue_id,
          mac->mbs.g_rnti);
  }
}

void nr_rrc_mac_config_other_sib(module_id_t module_id,
                                 ...
                                 bool can_start_ra)
{
  ...
  nr_ue_mac_config_mbs_sib20(mac, sib20);
