                           ...
                           const NR_SearchSpace_t *ss)
{
  ...
  int temp_num_dci_options = (mac->ra.ra_state == nrRA_WAIT_RAR || rnti_type == TYPE_SI_RNTI_ || rnti_type == TYPE_G_RNTI_) ? 1 : 2;
  int dci_format[2] = {0};
  if (ss->searchSpaceType->present == NR_SearchSpace__searchSpaceType_PR_ue_Specific) {
    ...
    */
    ...
    for (int i = 0; i < pdcch_config->list_SS.count; i++) {
      ...
      if (is_ss_monitor_occasion(frame, slot, slots_per_frame, ss)) {
        ...
        if (mac->mbs.enabled && ss->searchSpaceType->present == NR_SearchSpace__searchSpaceType_PR_common)
          config_dci_pdu(mac, dl_config, TYPE_G_RNTI_, slot, ss);
      }
    }
...
#undef CSS_VALID_CUR_SLOT
      ...
      if (mac->mbs.enabled)
        config_dci_pdu(mac, dl_config, TYPE_G_RNTI_, slot, css);
    }
  }
}
