...
static void nr_ue_mbs_deliver_payload(NR_UE_MAC_INST_t *mac, frame_t frame, int slot, uint8_t lcid, const uint8_t *buf, int len)
{
  if (len <= 0)
    return;

  mac->mbs.rx_pdus++;
  mac->mbs.rx_bytes += len;
  LOG_I(NR_MAC,
        "[UE %d][%d.%d] NR MBS MTCH SDU: G-RNTI 0x%04x LCID/MRB %u len %d totals pdus=%" PRIu64 " bytes=%" PRIu64 "\n",
        mac->ue_id,
        frame,
        slot,
        mac->mbs.g_rnti,
  ...
  const char *dump_path = getenv("OAI_NRUE_MBS_DUMP");
  if (dump_path && dump_path[0]) {
    ...
    if (dump) {
      ...
      LOG_W(NR_MAC, "Could not append NR MBS payload to %s\n", dump_path);
    }
  }

  const char *udp_target = getenv("OAI_NRUE_MBS_UDP");
  if (udp_target && udp_target[0]) {
    char target[128];
    strncpy(target, udp_target, sizeof(target) - 1);
    target[sizeof(target) - 1] = '\0';
    char *colon = strrchr(target, ':');
    if (!colon) {
      LOG_W(NR_MAC, "Invalid OAI_NRUE_MBS_UDP='%s', expected host:port\n", udp_target);
      return;
    }
    *colon = '\0';
    char *end = NULL;
    ...
    if (end == colon + 1 || port <= 0 || port > 65535) {
      LOG_W(NR_MAC, "Invalid OAI_NRUE_MBS_UDP port in '%s'\n", udp_target);
      return;
    }

    int fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (fd < 0) {
      LOG_W(NR_MAC, "Could not create NR MBS UDP egress socket\n");
      return;
    }
    ...
    if (inet_pton(AF_INET, target, &addr.sin_addr) == 1)
      ...
      LOG_W(NR_MAC, "Invalid OAI_NRUE_MBS_UDP IPv4 address '%s'\n", target);
    close(fd);
  }
}
...
int get_rnti_type(const NR_UE_MAC_INST_t *mac, const uint16_t rnti)
{
  ...
  } else if (mac->mbs.enabled && rnti == mac->mbs.g_rnti) {
    rnti_type = TYPE_G_RNTI_;
  } else if (rnti == 0xFFFE) {
                                   ...
                                   fapi_nr_dci_indication_pdu_t *dci_ind)
{
  ...
  const int ndi = rnti_type == TYPE_G_RNTI_ ? !current_harq->last_ndi : dci->ndi;
  ...
  if (rnti_type != TYPE_RA_RNTI_ && rnti_type != TYPE_SI_RNTI_ && rnti_type != TYPE_G_RNTI_) {
...
static bool extract_10_paging_style_rnti(dci_pdu_rel15_t *dci_pdu_rel15, const uint8_t *dci_pdu, int pos, const int N_RB)
{
  LOG_D(NR_MAC_DCI, "Received dci 1_0 paging-style rnti for MBS\n");

  int short_message_indicator = 0;
  int short_messages = 0;
  EXTRACT_DCI_ITEM(short_message_indicator, 2);
  EXTRACT_DCI_ITEM(short_messages, 8);
  if (short_message_indicator == 2)
    return false;
                                            ...
                                            const int ss_type)
{
  ...
  switch (rnti_type) {
    ...
    case TYPE_G_RNTI_:
      ...
      if (n_RB == 0)
        return NR_DCI_NONE;
      if (!extract_10_paging_style_rnti(dci_pdu_rel15, dci_pdu, pos, n_RB))
        return NR_DCI_NONE;
      break;
    case TYPE_SI_RNTI_ :
      format = NR_DL_DCI_FORMAT_1_0;
...
static void nr_ue_process_mac_pdu(NR_UE_MAC_INST_t *mac, nr_downlink_indication_t *dl_info, int pdu_id)
{
  ...
  uint8_t done = 0;
  RA_config_t *ra = &mac->ra;
  const bool mbs_pdu = pdsch_pdu->rnti_type == TYPE_G_RNTI_;
  ...
  while (!done && pdu_len > 0){
    ...
    switch (rx_lcid) {
      ...
      case 1 ... 32:
        ...
        if (mbs_pdu) {
          nr_ue_mbs_deliver_payload(mac, frameP, slot, rx_lcid, pduP + mac_subheader_len, mac_len);
        } else {
          nr_rlc_data_ind_t ind = {.ch = rx_lcid, .buf = pduP + mac_subheader_len, .len = mac_len};
          data_ind[num_data_ind++] = ind;
          DevAssert(num_data_ind < MAX_NUM_DATA_IND);
