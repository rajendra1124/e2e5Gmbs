...
void nr_ue_init_mac(NR_UE_MAC_INST_t *mac)
{
  ...
  memset(&mac->mbs, 0, sizeof(mac->mbs));

  const char *mbs_g_rnti = getenv("OAI_NRUE_MBS_G_RNTI");
  if (mbs_g_rnti) {
    char *end = NULL;
    unsigned long parsed = strtoul(mbs_g_rnti, &end, 0);
    if (end != mbs_g_rnti && parsed > 0 && parsed <= UINT16_MAX) {
      mac->mbs.enabled = true;
      mac->mbs.g_rnti = (uint16_t)parsed;
      LOG_I(NR_MAC, "[UE%d] NR MBS MTCH monitoring enabled for G-RNTI 0x%04x\n", mac->ue_id, mac->mbs.g_rnti);
    } else {
      LOG_W(NR_MAC, "[UE%d] Ignoring invalid OAI_NRUE_MBS_G_RNTI='%s'\n", mac->ue_id, mbs_g_rnti);
    }
  }
