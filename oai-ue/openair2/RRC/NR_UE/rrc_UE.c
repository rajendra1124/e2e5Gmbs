...
static void nr_decode_SI(NR_UE_RRC_SI_INFO *SI_info, NR_SystemInformation_t *si, NR_UE_RRC_INST_t *rrc, int hfn, int frame)
{
  ...
  for (int i = 0; i < si->criticalExtensions.choice.systemInformation->sib_TypeAndInfo.list.count; i++) {
    ...
    switch(typeandinfo->present) {
      ...
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib19_v1700:
        ...
        break;
      case NR_SystemInformation_IEs__sib_TypeAndInfo__Member_PR_sib20_v1700:
        sib20 = typeandinfo->choice.sib20_v1700;
        if (g_log->log_component[NR_RRC].level >= OAILOG_DEBUG)
          xer_fprint(stdout, &asn_DEF_NR_SIB20_r17, (const void *)sib20);
        SI_info->SInfo_r17.sib20_validity = true;
        nr_timer_start(&SI_info->SInfo_r17.sib20_timer);
        if (SI_info->SInfo_r17.sib20)
          ASN_STRUCT_FREE(asn_DEF_NR_SIB20_r17, SI_info->SInfo_r17.sib20);
        asn_copy(&asn_DEF_NR_SIB20_r17, (void **)&SI_info->SInfo_r17.sib20, sib20);
        LOG_I(NR_RRC,
              "[UE %ld] decoded NR MBS SIB20-r17: mcch-windowStartSlot %ld, mcch-modificationPeriod enum %ld\n",
              rrc->ue_id,
              sib20->mcch_Config_r17.mcch_WindowStartSlot_r17,
              sib20->mcch_Config_r17.mcch_ModificationPeriod_r17);
        break;
      default:
        break;
    }
  ...
  if (sib19 || sib20) {
    ...
    nr_mac_rrc_config_other_sib_t *other_sib_msg = &rrc_msg.payload.config_other_sib;
    if (sib19)
      asn_copy(&asn_DEF_NR_SIB19_r17, (void **)&other_sib_msg->sib19, sib19);
    if (sib20)
      asn_copy(&asn_DEF_NR_SIB20_r17, (void **)&other_sib_msg->sib20, sib20);
    other_sib_msg->hfn = hfn;
    other_sib_msg->frame = frame;
    other_sib_msg->can_start_ra = rrc->is_NTN_UE;
