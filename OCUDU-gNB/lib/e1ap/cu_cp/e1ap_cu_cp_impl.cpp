...
static void fill_global_mbs_session_id(global_mbs_session_id_s& asn1_session_id, const e1ap_mbs_session_id& session_id)
{
...
{
  std::vector<uint8_t> out = qos_flow_ids;
  if (out.empty()) {
    out.push_back(1);
  }
...
static qos_flow_qos_param_item_s make_default_mbs_qos_flow_param(uint8_t qos_flow_id)
{
  qos_flow_qos_param_item_s item;
  item.qos_flow_id = qos_flow_id;
  item.qos_flow_level_qos_params.qos_characteristics.set_non_dyn_5qi().five_qi = 9;
  item.qos_flow_level_qos_params.ngra_nalloc_retention_prio.prio_level         = 1;
  item.qos_flow_level_qos_params.ngra_nalloc_retention_prio.pre_emption_cap.value =
      pre_emption_cap_opts::shall_not_trigger_pre_emption;
  item.qos_flow_level_qos_params.ngra_nalloc_retention_prio.pre_emption_vulnerability.value =
      pre_emption_vulnerability_opts::not_pre_emptable;
  return item;
}

static mcmrb_setup_cfg_item_s make_mbs_mrb_setup_cfg(const e1ap_mbs_mrb_item& mrb)
{
  mcmrb_setup_cfg_item_s item;
  item.mrb_id                       = mrb.mrb_id;
  item.mbs_pdcp_cfg.pdcp_sn_size_ul = pdcp_sn_size_e::s_neg18;
  item.mbs_pdcp_cfg.pdcp_sn_size_dl = pdcp_sn_size_e::s_neg18;
  item.mbs_pdcp_cfg.rlc_mode        = rlc_mode_opts::rlc_um_unidirectional_dl;

  for (uint8_t qos_flow_id : make_mbs_qos_flow_ids(mrb.qos_flow_ids)) {
    item.qos_flow_qos_param_list.push_back(make_default_mbs_qos_flow_param(qos_flow_id));
  }
  return item;
}

static mcmrb_setup_modify_cfg_item_s make_mbs_mrb_setup_modify_cfg(const e1ap_mbs_mrb_item& mrb)
{
  mcmrb_setup_modify_cfg_item_s item;
  item.mrb_id                       = mrb.mrb_id;
  item.mbs_pdcp_cfg_present         = true;
  item.mbs_pdcp_cfg.pdcp_sn_size_ul = pdcp_sn_size_e::s_neg18;
  item.mbs_pdcp_cfg.pdcp_sn_size_dl = pdcp_sn_size_e::s_neg18;
  item.mbs_pdcp_cfg.rlc_mode        = rlc_mode_opts::rlc_um_unidirectional_dl;
...
void e1ap_cu_cp_impl::handle_mbs_bearer_context_setup_request(
    const e1ap_mbs_bearer_context_setup_request& request)
{
  e1ap_message e1ap_msg;
  e1ap_msg.pdu.set_init_msg().load_info_obj(ASN1_E1AP_ID_MC_BEARER_CONTEXT_SETUP);
  auto& setup_request = e1ap_msg.pdu.init_msg().value.mc_bearer_context_setup_request();

  setup_request->gnb_cu_cp_mbs_e1ap_id = request.cu_cp_mbs_e1ap_id;
  fill_global_mbs_session_id(setup_request->global_mbs_session_id, request.session_id);
  setup_request->mc_bearer_context_to_setup.snssai.sst.from_number(1);

  if (request.area_session_id.has_value()) {
    setup_request->mc_bearer_context_to_setup.ie_exts_present                 = true;
    setup_request->mc_bearer_context_to_setup.ie_exts.mbs_area_session_id_present = true;
    setup_request->mc_bearer_context_to_setup.ie_exts.mbs_area_session_id     = *request.area_session_id;
  }

  for (const auto& mrb : request.mrb_to_setup_list) {
    setup_request->mc_bearer_context_to_setup.mc_mrb_to_setup_list.push_back(make_mbs_mrb_setup_cfg(mrb));
  }

  logger.info("MBS CU-CP-E1AP-ID={}: Sending MCBearerContextSetupRequest nof_mrbs={}",
              request.cu_cp_mbs_e1ap_id,
              request.mrb_to_setup_list.size());
  pdu_notifier.on_new_message(e1ap_msg);
}

void e1ap_cu_cp_impl::handle_mbs_bearer_context_modification_request(
    const e1ap_mbs_bearer_context_modification_request& request)
{
  e1ap_message e1ap_msg;
  e1ap_msg.pdu.set_init_msg().load_info_obj(ASN1_E1AP_ID_MC_BEARER_CONTEXT_MOD);
  auto& mod_request = e1ap_msg.pdu.init_msg().value.mc_bearer_context_mod_request();

  mod_request->gnb_cu_cp_mbs_e1ap_id = request.cu_cp_mbs_e1ap_id;
  mod_request->gnb_cu_up_mbs_e1ap_id = request.cu_up_mbs_e1ap_id;

  for (const auto& mrb : request.mrb_to_setup_modify_list) {
    mod_request->mc_bearer_context_to_modify.mc_mrb_to_setup_modify_list.push_back(
        make_mbs_mrb_setup_modify_cfg(mrb));
  }
  for (uint16_t mrb_id : request.mrb_to_remove_list) {
    mod_request->mc_bearer_context_to_modify.mc_mrb_to_rem_list.push_back(mrb_id);
  }

  logger.info("MBS CU-CP-E1AP-ID={}: Sending MCBearerContextModificationRequest CU-UP-MBS-E1AP-ID={} "
              "nof_setup_modify_mrbs={} nof_remove_mrbs={}",
              request.cu_cp_mbs_e1ap_id,
              request.cu_up_mbs_e1ap_id,
              request.mrb_to_setup_modify_list.size(),
              request.mrb_to_remove_list.size());
  pdu_notifier.on_new_message(e1ap_msg);
}

void e1ap_cu_cp_impl::handle_mbs_bearer_context_release_command(
    const e1ap_mbs_bearer_context_release_command& command)
{
  e1ap_message e1ap_msg;
  e1ap_msg.pdu.set_init_msg().load_info_obj(ASN1_E1AP_ID_MC_BEARER_CONTEXT_RELEASE);
  auto& release_cmd = e1ap_msg.pdu.init_msg().value.mc_bearer_context_release_cmd();

  release_cmd->gnb_cu_cp_mbs_e1ap_id = command.cu_cp_mbs_e1ap_id;
  release_cmd->gnb_cu_up_mbs_e1ap_id = command.cu_up_mbs_e1ap_id;
  release_cmd->cause.set_radio_network() = cause_radio_network_opts::normal_release;

  logger.info("MBS CU-CP-E1AP-ID={}: Sending MCBearerContextReleaseCommand CU-UP-MBS-E1AP-ID={}",
              command.cu_cp_mbs_e1ap_id,
              command.cu_up_mbs_e1ap_id);
  pdu_notifier.on_new_message(e1ap_msg);
}
...
void e1ap_cu_cp_impl::handle_successful_outcome(const asn1::e1ap::successful_outcome_s& outcome)
{
  ...
  switch (outcome.value.type().value) {
    ...
    case successful_types::mc_bearer_context_setup_resp: {
      e1ap_mbs_bearer_context_setup_response response;
      response.success           = true;
      response.cu_cp_mbs_e1ap_id = outcome.value.mc_bearer_context_setup_resp()->gnb_cu_cp_mbs_e1ap_id;
      response.cu_up_mbs_e1ap_id = outcome.value.mc_bearer_context_setup_resp()->gnb_cu_up_mbs_e1ap_id;
      cu_cp_notifier.on_mbs_bearer_context_setup_response_received(response);
      logger.info("MBS CU-CP-E1AP-ID={}: Received MCBearerContextSetupResponse CU-UP-MBS-E1AP-ID={}",
                  outcome.value.mc_bearer_context_setup_resp()->gnb_cu_cp_mbs_e1ap_id,
                  outcome.value.mc_bearer_context_setup_resp()->gnb_cu_up_mbs_e1ap_id);
    } break;
    case successful_types::mc_bearer_context_mod_resp: {
      e1ap_mbs_bearer_context_modification_response response;
      response.success           = true;
      response.cu_cp_mbs_e1ap_id = outcome.value.mc_bearer_context_mod_resp()->gnb_cu_cp_mbs_e1ap_id;
      response.cu_up_mbs_e1ap_id = outcome.value.mc_bearer_context_mod_resp()->gnb_cu_up_mbs_e1ap_id;
      cu_cp_notifier.on_mbs_bearer_context_modification_response_received(response);
      logger.info("MBS CU-CP-E1AP-ID={}: Received MCBearerContextModificationResponse CU-UP-MBS-E1AP-ID={}",
                  response.cu_cp_mbs_e1ap_id,
                  response.cu_up_mbs_e1ap_id);
    } break;
    case successful_types::mc_bearer_context_release_complete: {
      e1ap_mbs_bearer_context_release_complete complete;
      complete.cu_cp_mbs_e1ap_id = outcome.value.mc_bearer_context_release_complete()->gnb_cu_cp_mbs_e1ap_id;
      complete.cu_up_mbs_e1ap_id = outcome.value.mc_bearer_context_release_complete()->gnb_cu_up_mbs_e1ap_id;
      cu_cp_notifier.on_mbs_bearer_context_release_complete_received(complete);
      logger.info("MBS CU-CP-E1AP-ID={}: Received MCBearerContextReleaseComplete CU-UP-MBS-E1AP-ID={}",
                  complete.cu_cp_mbs_e1ap_id,
                  complete.cu_up_mbs_e1ap_id);
    } break;
    default:
...
void e1ap_cu_cp_impl::handle_unsuccessful_outcome(const asn1::e1ap::unsuccessful_outcome_s& outcome)
{
  ...
  switch (outcome.value.type().value) {
    ...
    case unsuccessful_types::mc_bearer_context_setup_fail: {
      logger.warning("MBS CU-CP-E1AP-ID={}: Received MCBearerContextSetupFailure",
                     outcome.value.mc_bearer_context_setup_fail()->gnb_cu_cp_mbs_e1ap_id);
    } break;
    case unsuccessful_types::mc_bearer_context_mod_fail: {
      logger.warning("MBS CU-CP-E1AP-ID={}: Received MCBearerContextModificationFailure CU-UP-MBS-E1AP-ID={}",
                     outcome.value.mc_bearer_context_mod_fail()->gnb_cu_cp_mbs_e1ap_id,
                     outcome.value.mc_bearer_context_mod_fail()->gnb_cu_up_mbs_e1ap_id);
    } break;
    default:
