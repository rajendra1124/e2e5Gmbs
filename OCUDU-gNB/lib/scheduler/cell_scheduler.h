...
public:
  ...
  void handle_mbs_context_setup(const sched_mbs_context_setup_request& request);

  void handle_mbs_context_release(const sched_mbs_context_release_request& request);

  void handle_mbs_buffer_state_update(const sched_mbs_buffer_state_indication& bs);
...
private:
  ...
  void schedule_mbs(cell_resource_allocator& grid, slot_point sl_tx);
  ...
  struct mbs_sched_context {
    sched_mbs_context_setup_request  setup;
    std::map<uint16_t, unsigned> mrb_pending_bytes;
    uint64_t last_mcch_period_scheduled = std::numeric_limits<uint64_t>::max();
  };

  std::map<std::string, mbs_sched_context> mbs_contexts;
