...
/// \brief Selects a search space from the UE-dedicated configuration for fallback C-RNTI grants, if usable.
///
/// Some UEs only monitor the RA search space (Type1-PDCCH CSS) for C-RNTI PDCCH while a RA procedure is ongoing,
/// even though TS 38.213, Section 10.1 lists the C-RNTI among the RNTIs monitored in this search space. Grants
/// sent in the RA search space after the ConRes procedure completion may therefore never be decoded by such UEs,
/// preventing e.g. the delivery of the RRC Setup Complete. Once ConRes is complete (at which point the UE has
/// applied the CellGroupConfig carried in the RRC Setup), prefer a search space from the UE-dedicated
/// configuration.
static const search_space_info* get_ded_cfg_fallback_search_space(const ue& u, slot_point pdcch_slot)
{
  if (u.ue_cfg_dedicated() == nullptr or not u.get_pcell().get_pcell_state().conres_complete) {
    return nullptr;
  }
  const ue_cell_configuration& ue_cc_cfg = u.get_pcell().cfg();
  const auto& common_ss_list = ue_cc_cfg.cell_cfg_common.params.dl_cfg_common.init_dl_bwp.pdcch_common.search_spaces;
  for (const sched_search_space_config* ss_ded :
       ue_cc_cfg.bwp(u.get_pcell().active_bwp_id()).dl.pdcch().search_spaces()) {
    const bool is_cell_common_ss =
        std::any_of(common_ss_list.begin(), common_ss_list.end(), [&ss_ded](const search_space_configuration& css) {
          return css.get_id() == ss_ded->id();
        });
    if (is_cell_common_ss) {
      continue;
    }
    const search_space_info& ss = ue_cc_cfg.search_space(ss_ded->id());
    if (ss.get_ul_dci_format() != dci_ul_format::f0_0 or ss.get_dl_dci_format() != dci_dl_format::f1_0) {
      // In fallback, only DCI formats 0_0/1_0 are used.
      continue;
    }
    if (not pdcch_helper::is_pdcch_monitoring_active(pdcch_slot, *ss.cfg) or
        ss.get_pdcch_candidates(aggregation_level::n4, pdcch_slot).empty()) {
      continue;
    }
    return &ss;
  }
                                   ...
                                   std::optional<dl_harq_process_handle> h_dl_retx)
{
  ...
  // For C-RNTI allocations after ConRes completion, prefer a search space from the UE-dedicated configuration, as
  // some UEs stop monitoring the RA search space for C-RNTI when no RA procedure is ongoing.
  const search_space_info* ded_ss = dci_type == dci_dl_rnti_config_type::c_rnti_f1_0
                                        ? get_ded_cfg_fallback_search_space(u, res_alloc[slot_offset].slot)
                                        : nullptr;
  ...
  // Allocate PDCCH resources. Use the UE-dedicated configured search space when available, so that UEs that only
  // monitor the RA search space during an ongoing RA procedure can receive the grant.
  cell_slot_resource_allocator& pdcch_alloc = res_alloc[slot_offset];
  pdcch_dl_information*         pdcch =
      ded_ss != nullptr
          ? pdcch_sch.alloc_dl_pdcch_ue(
                pdcch_alloc, u.crnti, u.get_pcell().cfg(), ded_ss->cfg->get_id(), aggregation_level::n4)
                                                                                  ...
                                                                                  ue&                      u)
{
  ...
  // Prefer a search space from the UE-dedicated configuration once the ConRes procedure is complete, as some UEs
  // stop monitoring the RA search space for C-RNTI when no RA procedure is ongoing.
  const search_space_info* ded_ss = get_ded_cfg_fallback_search_space(u, pdcch_slot);
  if (ded_ss != nullptr) {
    search_spaces.clear();
    search_spaces.push_back(ded_ss);
  }
  ...
  for (const auto* ss : search_spaces) {
    for (unsigned pusch_td_res_idx : pusch_td_res_index_list) {
      ...
      ul_srb_sched_outcome outcome =
          schedule_ul_srb(u, res_alloc, pusch_td_res_idx, pusch_td, h_ul_retx, remove_pucch, ded_ss);
...
ue_fallback_scheduler::schedule_ul_srb(ue&                                          u,
                                       ...
                                       bool                                         remove_pucch,
                                       const search_space_info*                     ded_ss)
{
  ue_cell&                      ue_pcell    = u.get_pcell();
  ...
  // Allocate PDCCH position. Use the UE-dedicated configured search space when provided, so that UEs that only
  // monitor the RA search space during an ongoing RA procedure can receive the grant.
  pdcch_ul_information* pdcch =
      ded_ss != nullptr
          ? pdcch_sch.alloc_ul_pdcch_ue(
                pdcch_alloc, u.crnti, u.get_pcell().cfg(), ded_ss->cfg->get_id(), aggregation_level::n4)
