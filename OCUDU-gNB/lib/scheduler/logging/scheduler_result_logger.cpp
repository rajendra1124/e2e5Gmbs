...
static auto make_mbs_info_log_entry(const dl_mbs_allocation& mbs_info)
{
  return make_formattable([rb      = mbs_info.pdsch_cfg.rbs,
                           tbs     = mbs_info.pdsch_cfg.codewords[0].tb_size_bytes,
                           rnti    = mbs_info.pdsch_cfg.rnti,
                           channel = mbs_info.channel](auto& ctx) {
    return fmt::format_to(ctx.out(),
                          "{}: rnti={} rb={} tbs={}",
                          channel == dl_mbs_allocation::channel_type::mcch ? "MCCH" : "MTCH",
                          rnti,
                          rb,
                          tbs);
  });
...
static auto make_info_log_entry(const sched_result& result, bool log_broadcast)
{
  ...
  using mbs_entry_t    = decltype(make_mbs_info_log_entry(std::declval<dl_mbs_allocation>()));

  auto entries = type_list_buffer_stream<sib_entry_t, rar_entry_t, ue_dl_entry_t, ue_ul_entry_t, paging_entry_t, mbs_entry_t>::make(
  ...
  for (const auto& mbs : result.dl.mbs_grants) {
    entries.push(make_mbs_info_log_entry(mbs));
  }
...
void scheduler_result_logger::log_debug(const sched_result& result, std::chrono::microseconds decision_latency)
{
  ...
  const bool non_broadcast_is_empty =
      ...
      result.dl.mbs_grants.empty() and result.dl.ue_grants.empty() and result.ul.puschs.empty() and
      result.ul.pucchs.empty() and result.ul.srss.empty();
  ...
  if (slot_is_logged) {
    unsigned nof_pdschs = result.dl.paging_grants.size() + result.dl.mbs_grants.size() +
                           result.dl.rar_grants.size() + result.dl.ue_grants.size();
    if (log_broadcast) {
...
void scheduler_result_logger::log_info(const sched_result& result, std::chrono::microseconds decision_latency)
{
  ...
  bool       slot_is_logged  = (log_broadcast and not result.dl.bc.sibs.empty()) or not result.dl.rar_grants.empty() or
                        ...
                        not result.dl.paging_grants.empty() or not result.dl.mbs_grants.empty() or failed_attempts;
