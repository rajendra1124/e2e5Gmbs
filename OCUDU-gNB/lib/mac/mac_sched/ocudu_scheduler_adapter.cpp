...
void ocudu_scheduler_adapter::handle_mbs_context_setup(const mac_mbs_context_setup_request& request)
{
  sched_mbs_context_setup_request sched_request;
  sched_request.session_key         = request.session_key;
  sched_request.gnb_cu_mbs_f1ap_id  = request.gnb_cu_mbs_f1ap_id;
  sched_request.gnb_du_mbs_f1ap_id  = request.gnb_du_mbs_f1ap_id;
  sched_request.mcch_rnti           = request.mcch_rnti;
  sched_request.mtch_rnti           = request.mtch_rnti;
  sched_request.mcch_repetition_period_rf  = request.mcch_repetition_period_rf;
  sched_request.mcch_repetition_offset_rf  = request.mcch_repetition_offset_rf;
  sched_request.mcch_window_start_slot     = request.mcch_window_start_slot;
  sched_request.mcch_window_duration_slots = request.mcch_window_duration_slots;
  sched_request.mcch_payload_size_bytes    = request.mcch_payload_size_bytes;
  sched_request.mrb_ids             = request.mrb_ids;
  sched_impl->handle_mbs_context_setup(sched_request);
}

void ocudu_scheduler_adapter::handle_mbs_context_release(const mac_mbs_context_release_request& request)
{
  sched_mbs_context_release_request sched_request;
  sched_request.session_key        = request.session_key;
  sched_request.gnb_cu_mbs_f1ap_id = request.gnb_cu_mbs_f1ap_id;
  sched_request.gnb_du_mbs_f1ap_id = request.gnb_du_mbs_f1ap_id;
  sched_impl->handle_mbs_context_release(sched_request);
}

void ocudu_scheduler_adapter::handle_mbs_buffer_state_update(const mac_mbs_buffer_state_indication& bs)
{
  sched_mbs_buffer_state_indication sched_bs;
  sched_bs.session_key   = bs.session_key;
  sched_bs.mrb_id        = bs.mrb_id;
  sched_bs.pending_bytes = bs.pending_bytes;
  sched_impl->handle_mbs_buffer_state_update(sched_bs);
}

void ocudu_scheduler_adapter::handle_mbs_data_pdu(mac_mbs_data_indication msg)
{
  // MBS user-plane buffering is owned by MAC DL. The scheduler currently allocates MTCH periodically from context.
}
