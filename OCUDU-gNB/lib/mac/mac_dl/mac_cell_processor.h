...
public:
  ...
  void handle_mbs_context_setup(const mac_mbs_context_setup_request& request);

  void handle_mbs_context_release(const mac_mbs_context_release_request& request);

  void handle_mbs_data_pdu(mac_mbs_data_indication msg);
...
private:
  ...
  byte_buffer encode_mbs_mcch_pdu(const dl_mbs_allocation& mbs) const;

  byte_buffer encode_mbs_mtch_pdu(const dl_mbs_allocation& mbs, size_t tbs_bytes);
  ...
  struct mbs_cell_context {
    mac_mbs_context_setup_request      setup;
    std::map<uint16_t, mbs_mrb_bearer> mrbs;
  };

  std::map<std::string, mbs_cell_context> mbs_contexts;
};
