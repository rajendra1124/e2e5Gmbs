...
public:
  ...
  /// Handle setup/update of an MCCH/MTCH MBS radio context in the scheduler.
  virtual void handle_mbs_context_setup(const mac_mbs_context_setup_request& request) = 0;

  /// Handle release of an MCCH/MTCH MBS radio context in the scheduler.
  virtual void handle_mbs_context_release(const mac_mbs_context_release_request& request) = 0;

  /// Handle MTCH buffered-byte updates for MBS radio scheduling.
  virtual void handle_mbs_buffer_state_update(const mac_mbs_buffer_state_indication& bs) = 0;
};
