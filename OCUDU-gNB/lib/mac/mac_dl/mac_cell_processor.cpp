...
static std::string make_mbs_service_id_hex(std::string_view session_key)
{
  std::string hex;
  hex.reserve(6);
...
static uint8_t mbs_mrb_id_to_lcid(uint16_t mrb_id)
{
  return static_cast<uint8_t>(std::clamp<unsigned>(mrb_id == 0 ? 1U : mrb_id, 1U, 32U));
}

static mac_mbs_mrb_config get_mbs_mrb_config_or_default(const mac_mbs_context_setup_request& setup, uint16_t mrb_id)
{
  auto it = std::find_if(setup.mrb_configs.begin(), setup.mrb_configs.end(), [mrb_id](const mac_mbs_mrb_config& cfg) {
    return cfg.mrb_id == mrb_id;
  });
  if (it != setup.mrb_configs.end()) {
    return *it;
  }
  mac_mbs_mrb_config default_cfg;
  default_cfg.mrb_id = mrb_id;
  return default_cfg;
}

static bool mbs_mrb_configs_equal(const mac_mbs_mrb_config& lhs, const mac_mbs_mrb_config& rhs)
{
  return lhs.mrb_id == rhs.mrb_id and lhs.pdcp_sn_size_dl == rhs.pdcp_sn_size_dl and
         lhs.pdcp_t_reordering_dl == rhs.pdcp_t_reordering_dl and lhs.rlc_sn_size == rhs.rlc_sn_size and
         lhs.rlc_t_reassembly_ms == rhs.rlc_t_reassembly_ms;
}

static void fill_mbs_broadcast_mrb_info(asn1::rrc_nr::mrb_info_broadcast_r17_s& mrb,
                                        const mac_mbs_mrb_config&               cfg)
{
  mrb.pdcp_cfg_r17.hdr_compress_r17.set_not_used();
  mrb.pdcp_cfg_r17.pdcp_sn_size_dl_r17_present = cfg.pdcp_sn_size_dl == pdcp_sn_size::size12bits;
  mrb.pdcp_cfg_r17.t_reordering_r17_present    = true;
  switch (cfg.pdcp_t_reordering_dl) {
    case pdcp_t_reordering::ms1:
      mrb.pdcp_cfg_r17.t_reordering_r17 =
          asn1::rrc_nr::mrb_pdcp_cfg_broadcast_r17_s::t_reordering_r17_opts::ms1;
      break;
    case pdcp_t_reordering::ms10:
      mrb.pdcp_cfg_r17.t_reordering_r17 =
          asn1::rrc_nr::mrb_pdcp_cfg_broadcast_r17_s::t_reordering_r17_opts::ms10;
      break;
    case pdcp_t_reordering::ms160:
      mrb.pdcp_cfg_r17.t_reordering_r17 =
          asn1::rrc_nr::mrb_pdcp_cfg_broadcast_r17_s::t_reordering_r17_opts::ms160;
      break;
    case pdcp_t_reordering::ms500:
      mrb.pdcp_cfg_r17.t_reordering_r17 =
          asn1::rrc_nr::mrb_pdcp_cfg_broadcast_r17_s::t_reordering_r17_opts::ms500;
      break;
    case pdcp_t_reordering::ms1000:
      mrb.pdcp_cfg_r17.t_reordering_r17 =
          asn1::rrc_nr::mrb_pdcp_cfg_broadcast_r17_s::t_reordering_r17_opts::ms1000;
      break;
    case pdcp_t_reordering::ms1250:
      mrb.pdcp_cfg_r17.t_reordering_r17 =
          asn1::rrc_nr::mrb_pdcp_cfg_broadcast_r17_s::t_reordering_r17_opts::ms1250;
      break;
    case pdcp_t_reordering::ms2750:
      mrb.pdcp_cfg_r17.t_reordering_r17 =
          asn1::rrc_nr::mrb_pdcp_cfg_broadcast_r17_s::t_reordering_r17_opts::ms2750;
      break;
    case pdcp_t_reordering::ms40:
    default:
      mrb.pdcp_cfg_r17.t_reordering_r17 =
          asn1::rrc_nr::mrb_pdcp_cfg_broadcast_r17_s::t_reordering_r17_opts::ms40;
      break;
  }
  mrb.rlc_cfg_r17.lc_ch_id_r17                 = mbs_mrb_id_to_lcid(cfg.mrb_id);
  mrb.rlc_cfg_r17.sn_field_len_r17_present     = cfg.rlc_sn_size == rlc_um_sn_size::size6bits;
  mrb.rlc_cfg_r17.t_reassembly_r17_present     = true;
  mrb.rlc_cfg_r17.t_reassembly_r17             = asn1::rrc_nr::t_reassembly_opts::ms40;
}
...
void mac_cell_processor::handle_mbs_context_setup(const mac_mbs_context_setup_request& request)
{
  defer_until_success(cell_exec, timers, [this, request]() {
    auto& ctx = mbs_contexts[request.session_key];
    mac_mbs_context_setup_request old_setup = ctx.setup;
    ctx.setup = request;
    for (auto it = ctx.mrbs.begin(); it != ctx.mrbs.end();) {
      const bool removed = std::find(request.mrb_ids.begin(), request.mrb_ids.end(), it->first) == request.mrb_ids.end();
      const bool config_changed =
          not removed and not mbs_mrb_configs_equal(get_mbs_mrb_config_or_default(request, it->first),
                                                    get_mbs_mrb_config_or_default(old_setup, it->first));
      if (removed or config_changed) {
        it = ctx.mrbs.erase(it);
      } else {
        ++it;
      }
    }
    for (uint16_t mrb_id : request.mrb_ids) {
      ctx.mrbs.try_emplace(
          mrb_id,
          mrb_id,
          get_mbs_mrb_config_or_default(request, mrb_id),
          timers,
          cell_exec,
          cell_exec,
          [this, session_key = request.session_key, mrb_id](const rlc_buffer_state& bs) {
            sched.handle_mbs_buffer_state_update({session_key, mrb_id, bs.pending_bytes});
          });
      sched.handle_mbs_buffer_state_update({request.session_key, mrb_id, 0});
    }
    logger.info("cell={}: MBS radio context installed. session={} mcch-rnti={} mtch-rnti={} nof-mrbs={}",
                fmt::underlying(cell_cfg.cell_index),
                request.session_key,
                request.mcch_rnti,
                request.mtch_rnti,
...
void mac_cell_processor::handle_mbs_context_release(const mac_mbs_context_release_request& request)
{
  defer_until_success(cell_exec, timers, [this, request]() {
    mbs_contexts.erase(request.session_key);
    logger.info("cell={}: MBS radio context released. session={}",
                fmt::underlying(cell_cfg.cell_index),
                request.session_key);
  });
}

void mac_cell_processor::handle_mbs_data_pdu(mac_mbs_data_indication msg)
{
  defer_until_success(cell_exec, timers, [this, msg = std::move(msg)]() mutable {
    auto ctx_it = mbs_contexts.find(msg.session_key);
    if (ctx_it == mbs_contexts.end()) {
      logger.warning("cell={}: Dropping MBS MTCH PDU for unknown session={}",
                     fmt::underlying(cell_cfg.cell_index),
                     msg.session_key);
      return;
    }
    auto mrb_it = ctx_it->second.mrbs.find(msg.mrb_id);
    if (mrb_it == ctx_it->second.mrbs.end()) {
      logger.warning("cell={}: Dropping MBS MTCH PDU for unknown MRB. session={} mrb={}",
                     fmt::underlying(cell_cfg.cell_index),
                     msg.session_key,
                     msg.mrb_id);
      return;
    }
    mrb_it->second.push_sdu(std::move(msg.pdu));
    sched.handle_mbs_buffer_state_update({ctx_it->first, msg.mrb_id, mrb_it->second.pending()});
  });
}
    ...
    OCUDU_RTSAN_NONBLOCKING
{
  ...
  if (sl_res.dl.nof_dl_symbols > 0) {
    ...
    if (not sl_res.dl.ue_grants.empty() or not sl_res.dl.rar_grants.empty() or not sl_res.dl.bc.sibs.empty() or
        not sl_res.dl.paging_grants.empty() or not sl_res.dl.mbs_grants.empty()) {
...
byte_buffer mac_cell_processor::encode_mbs_mcch_pdu(const dl_mbs_allocation& mbs) const
{
  using namespace asn1::rrc_nr;

  mcch_msg_r17_s mcch;
  auto&          cfg = mcch.msg.set_c1().set_mbs_broadcast_cfg_r17().crit_exts.set_mbs_broadcast_cfg_r17();

  const auto ctx_it = mbs_contexts.find(mbs.session_key);
  const std::vector<uint16_t>* mrb_ids =
      ctx_it == mbs_contexts.end() ? nullptr : &ctx_it->second.setup.mrb_ids;
  const mac_mbs_context_setup_request* setup = ctx_it == mbs_contexts.end() ? nullptr : &ctx_it->second.setup;

  cfg.mbs_session_info_list_r17.push_back(mbs_session_info_r17_s{});
  mbs_session_info_r17_s& session = cfg.mbs_session_info_list_r17.back();
  session.mbs_session_id_r17.plmn_id_r17.set_plmn_idx() = 1;
  session.mbs_session_id_r17.service_id_r17.from_string(make_mbs_service_id_hex(mbs.session_key));
  session.g_rnti_r17                         = to_value(mbs.pdsch_cfg.rnti);
  session.mtch_sched_info_r17_present        = true;
  session.mtch_sched_info_r17                = 0;
  session.pdsch_cfg_idx_r17_present          = true;
  session.pdsch_cfg_idx_r17                  = 0;
  session.mtch_ssb_map_win_idx_r17_present   = true;
  session.mtch_ssb_map_win_idx_r17           = 0;

  if (mrb_ids == nullptr or mrb_ids->empty()) {
    session.mrb_list_broadcast_r17.push_back(mrb_info_broadcast_r17_s{});
    mac_mbs_mrb_config mrb_cfg;
    mrb_cfg.mrb_id = mbs.mrb_id;
    fill_mbs_broadcast_mrb_info(session.mrb_list_broadcast_r17.back(), mrb_cfg);
  } else {
    const unsigned nof_mrbs = std::min<unsigned>(mrb_ids->size(), 4U);
    for (unsigned i = 0; i != nof_mrbs; ++i) {
      session.mrb_list_broadcast_r17.push_back(mrb_info_broadcast_r17_s{});
      fill_mbs_broadcast_mrb_info(session.mrb_list_broadcast_r17.back(),
                                  get_mbs_mrb_config_or_default(*setup, (*mrb_ids)[i]));
    }
  }

  byte_buffer   pdu;
  asn1::bit_ref bref{pdu};
  if (mcch.pack(bref) != asn1::OCUDUASN_SUCCESS) {
    logger.error("Failed to ASN.1 encode MCCH-Message-r17 for MBS session={}", mbs.session_key);
    return byte_buffer{};
  }
  return pdu;
}

byte_buffer mac_cell_processor::encode_mbs_mtch_pdu(const dl_mbs_allocation& mbs, size_t tbs_bytes)
{
  auto ctx_it = mbs_contexts.find(mbs.session_key);
  if (ctx_it == mbs_contexts.end()) {
    logger.warning("No MBS context for scheduled MTCH session={}", mbs.session_key);
    return byte_buffer{};
  }

  auto mrb_it = ctx_it->second.mrbs.find(mbs.mrb_id);
  if (mrb_it == ctx_it->second.mrbs.end() or mrb_it->second.empty()) {
    logger.debug("No queued MTCH payload for MBS session={} mrb={}", mbs.session_key, mbs.mrb_id);
    return byte_buffer{};
  }

  byte_buffer pdu = mrb_it->second.pull_pdu(tbs_bytes);
  sched.handle_mbs_buffer_state_update({ctx_it->first, mbs.mrb_id, mrb_it->second.pending()});
  return pdu;
}
                                                  ...
                                                  const dl_sched_result& dl_res)
{
  ...
  // Assemble scheduled MBS MCCH/MTCH payloads.
  for (const dl_mbs_allocation& mbs : dl_res.mbs_grants) {
    for (unsigned cw_idx = 0, e = mbs.pdsch_cfg.codewords.size(); cw_idx != e; ++cw_idx) {
      const unsigned tbs     = mbs.pdsch_cfg.codewords[cw_idx].tb_size_bytes.value();
      span<uint8_t>  tb_data = pdu_pool.allocate_buffer(tbs);
      std::fill(tb_data.begin(), tb_data.end(), 0);

      byte_buffer payload;
      size_t      copied = 0;
      if (mbs.channel == dl_mbs_allocation::channel_type::mcch) {
        payload = encode_mbs_mcch_pdu(mbs);
        copied  = copy_segments(payload, tb_data);
      } else if (tbs > MIN_MAC_SDU_SUBHEADER_SIZE) {
        // MTCH carries an MRB RLC PDU as a variable-length MAC SDU. The MAC subheader is required by UEs to recover
        // the MRB LCID and exact RLC PDU length instead of interpreting the first RLC header byte as an LCID.
        const unsigned max_rlc_pdu_size = get_mac_sdu_payload_size(tbs);
        payload                         = encode_mbs_mtch_pdu(mbs, max_rlc_pdu_size);
        if (not payload.empty()) {
          dl_sch_pdu mac_pdu(tb_data);
          copied = mac_pdu.add_sdu(uint_to_lcid(mbs_mrb_id_to_lcid(mbs.mrb_id)), payload);
          if (mac_pdu.nof_empty_bytes() > 0) {
            mac_pdu.add_padding(mac_pdu.nof_empty_bytes());
          }
        }
      }
      logger.debug("Encoded MBS {} payload. session={} mrb={} bytes={} tbs={}",
                   mbs.channel == dl_mbs_allocation::channel_type::mcch ? "MCCH" : "MTCH",
                   mbs.session_key,
                   mbs.mrb_id,
                   copied,
                   tb_data.size());
      data_res.mbs_pdus.emplace_back(cw_idx, shared_transport_block(tb_data));
    }
  }
}
                                           ...
                                           const mac_dl_data_result& dl_res)
{
  ...
  for (unsigned i = 0, e = dl_res.mbs_pdus.size(); i != e; ++i) {
    const mac_dl_data_result::dl_pdu& mbs_pdu  = dl_res.mbs_pdus[i];
    const dl_mbs_allocation&          dl_alloc = sl_res.dl.mbs_grants[i];
    ocudu::mac_nr_context_info        context  = {};
    context.radioType           = cell_cfg.sched_req.ran.tdd_cfg.has_value() ? PCAP_TDD_RADIO : PCAP_FDD_RADIO;
    context.direction           = PCAP_DIRECTION_DOWNLINK;
    context.rntiType            = PCAP_G_RNTI;
    context.rnti                = to_value(dl_alloc.pdsch_cfg.rnti);
    context.system_frame_number = sl_tx.sfn();
    context.sub_frame_number    = sl_tx.subframe_index();
    context.length              = mbs_pdu.pdu.get_buffer().size();
    pcap.push_pdu(context, mbs_pdu.pdu.get_buffer());
  }
