...
public:
  ...
  // f1ap_mbs_manager functions.
  void handle_mbs_session_update_request(const f1ap_mbs_session_update_request& request) override;
  void handle_mbs_session_release_request(const f1ap_mbs_session_release_request& request) override;
  ...
  f1ap_mbs_manager&                  get_f1ap_mbs_manager() override { return *this; }
...
private:
  ...
  struct mbs_context {
    ...
    std::optional<uint64_t> du_mbs_f1ap_id;
    std::string            distribution_context_ref;
    bool                   setup_sent             = false;
    bool                   distribution_setup_sent = false;
    bool                   distribution_update_pending = false;
    /// NG-U shared multicast tunnel signalled by the 5GC. When present, it is threaded into the F1AP multicast
    /// distribution setup as the MRB traffic source, so the DU can ingest the multicast traffic directly.
    std::optional<up_transport_layer_info> ngu_tnl_info;
  };

  std::string make_mbs_session_key(const cu_cp_mbs_session_id& session_id) const;
  uint64_t    allocate_cu_mbs_f1ap_id();
  void        send_mbs_distribution_setup_request(const std::string& key,
                                                  mbs_context&       context,
                                                  const std::vector<uint16_t>& mrb_ids);
  void        send_mbs_distribution_release_command(const std::string& key, const mbs_context& context);
  ...
  // Repository of MBS contexts for this DU F1 interface.
  std::map<std::string, mbs_context> mbs_contexts;
  uint64_t                           next_cu_mbs_f1ap_id = 1;
};
