// SPDX-FileCopyrightText: Copyright (C) 2021-2026 Software Radio Systems Limited
// SPDX-License-Identifier: BSD-3-Clause-Open-MPI
// Portions of this file may implement 3GPP specifications, which may be subject to additional licensing requirements.

#include "f1ap_du_impl.h"
#include "../f1ap_asn1_utils.h"
#include "asn1_helpers.h"
#include "du/procedures/f1ap_du_positioning_information_exchange_procedure.h"
#include "du/procedures/f1ap_du_positioning_measurement_procedure.h"
#include "du/procedures/f1ap_du_trp_information_exchange_procedure.h"
#include "f1ap_du_connection_handler.h"
#include "log_helpers.h"
#include "procedures/f1ap_du_gnbdu_config_update_procedure.h"
#include "procedures/f1ap_du_initiated_reset_procedure.h"
#include "procedures/f1ap_du_paging_procedure.h"
#include "procedures/f1ap_du_removal_procedure.h"
#include "procedures/f1ap_du_reset_procedure.h"
#include "procedures/f1ap_du_setup_procedure.h"
#include "procedures/f1ap_du_ue_context_release_procedure.h"
#include "procedures/f1ap_du_ue_context_setup_procedure.h"
#include "procedures/gnb_cu_configuration_update_procedure.h"
#include "ue_context/f1ap_du_ue_config_update.h"
#include "ocudu/asn1/f1ap/common.h"
#include "ocudu/asn1/f1ap/f1ap.h"
#include "ocudu/asn1/f1ap/f1ap_pdu_items.h"
#include "ocudu/f1ap/f1ap_message.h"
#include "ocudu/gtpu/gtpu_config.h"
#include "ocudu/gtpu/gtpu_teid_pool_factory.h"
#include "ocudu/ran/nr_cgi.h"
#include "ocudu/ran/pcch/paging_helper.h"
#include "ocudu/ran/up_transport_layer_info.h"
#include <algorithm>

using namespace ocudu;
using namespace asn1::f1ap;
using namespace odu;

class f1ap_du_impl::tx_pdu_notifier_with_logging final : public f1ap_message_notifier
{
public:
  tx_pdu_notifier_with_logging(f1ap_du_impl& parent_, std::unique_ptr<f1ap_message_notifier> decorated_) :
    parent(parent_), decorated(std::move(decorated_))
  {
  }

  void on_new_message(const f1ap_message& msg) override
  {
    // Log message.
    parent.log_pdu(false, msg);

    // Forward message to DU.
    decorated->on_new_message(msg);
  }

private:
  f1ap_du_impl&                          parent;
  std::unique_ptr<f1ap_message_notifier> decorated;
};

f1ap_du_impl::f1ap_du_impl(f1c_connection_client&   f1c_client_handler_,
                           f1ap_du_configurator&    du_mng_,
                           task_executor&           ctrl_exec_,
                           f1ap_ue_executor_mapper& ue_exec_mapper_,
                           f1ap_du_paging_notifier& paging_notifier_,
                           timer_manager&           timers_,
                           gtpu_demux_ctrl*         mbs_demux_ctrl_,
                           task_executor*           mbs_f1u_exec_) :
  logger(ocudulog::fetch_basic_logger("DU-F1")),
  ctrl_exec(ctrl_exec_),
  du_mng(du_mng_),
  paging_notifier(paging_notifier_),
  timers(timers_),
  connection_handler(f1c_client_handler_, *this, du_mng, ctxt, ctrl_exec),
  ues(du_mng, ctrl_exec, ue_exec_mapper_, timers_),
  events(std::make_unique<f1ap_event_manager>(du_mng.get_timer_factory())),
  metrics(true)
{
  mbs_demux_ctrl = mbs_demux_ctrl_;
  mbs_f1u_exec   = mbs_f1u_exec_;
  gtpu_allocator_creation_request mbs_teid_pool_cfg = {};
  mbs_teid_pool_cfg.max_nof_teids                   = 4096;
  mbs_f1u_teid_pool                                 = create_gtpu_allocator(mbs_teid_pool_cfg);
  mbs_f1u_router                                    = std::make_unique<f1u_mbs_router>(*mbs_f1u_teid_pool);
}

// Note: For fwd declaration of member types, dtor cannot be trivial.
f1ap_du_impl::~f1ap_du_impl() {}

bool f1ap_du_impl::connect_to_cu_cp()
{
  std::unique_ptr<f1ap_message_notifier> pdu_notifier = connection_handler.connect_to_cu_cp();
  if (pdu_notifier == nullptr) {
    return false;
  }

  if (logger.info.enabled()) {
    // Decorate notifier with logging, if the logger is enabled.
    tx_pdu_notifier.reset(new tx_pdu_notifier_with_logging(*this, std::move(pdu_notifier)));
  } else {
    tx_pdu_notifier = std::move(pdu_notifier);
  }

  // Update other components Tx PDU notifier.
  ues.update_tx_pdu_notifier(*tx_pdu_notifier);

  return true;
}

async_task<void> f1ap_du_impl::disconnect_from_cu_cp()
{
  return connection_handler.handle_tnl_association_removal();
}

async_task<f1_setup_result> f1ap_du_impl::handle_f1_setup_request(const f1_setup_request_message& request)
{
  return launch_async<f1ap_du_setup_procedure>(request, *tx_pdu_notifier, *events, du_mng.get_timer_factory(), ctxt);
}

async_task<void> f1ap_du_impl::handle_f1_removal_request()
{
  return launch_async<f1ap_du_removal_procedure>(connection_handler, *tx_pdu_notifier, *events, ctxt);
}

async_task<f1_reset_acknowledgement> f1ap_du_impl::handle_f1_reset_request(const f1_reset_request& req)
{
  return launch_async<f1ap_du_initiated_reset_procedure>(req, *tx_pdu_notifier, *events, ues);
}

async_task<gnbdu_config_update_response>
f1ap_du_impl::handle_du_config_update(const gnbdu_config_update_request& request)
{
  return launch_async<f1ap_du_gnbdu_config_update_procedure>(request, *tx_pdu_notifier, *events);
}

f1ap_ue_creation_response f1ap_du_impl::handle_ue_creation_request(const f1ap_ue_creation_request& msg)
{
  f1ap_ue_creation_response resp = create_f1ap_ue(msg, ues, ctxt, *events, metrics);
  if (resp.result) {
    logger.info("{}: F1 UE context created successfully.", ues[msg.ue_index].context);
  } else {
    logger.warning("ue={} crnti={}: F1 UE context failed to be created.", fmt::underlying(msg.ue_index), msg.c_rnti);
  }
  return resp;
}

f1ap_ue_configuration_response f1ap_du_impl::handle_ue_configuration_request(const f1ap_ue_configuration_request& msg)
{
  return update_f1ap_ue_config(msg, ues);
}

void f1ap_du_impl::handle_ue_deletion_request(du_ue_index_t ue_index)
{
  if (ues.contains(ue_index)) {
    logger.info("{}: F1 UE context removed.", ues[ue_index].context);
  }
  ues.remove_ue(ue_index);
  metrics.on_ue_removal(ue_index);
}

void f1ap_du_impl::handle_reset(const reset_s& msg)
{
  du_mng.schedule_async_task(launch_async<reset_procedure>(msg, du_mng, ues, *tx_pdu_notifier));
}

void f1ap_du_impl::handle_gnb_cu_configuration_update(const gnb_cu_cfg_upd_s& msg)
{
  du_mng.schedule_async_task(launch_async<gnb_cu_configuration_update_procedure>(msg, du_mng, *tx_pdu_notifier));
}

void f1ap_du_impl::handle_ue_context_setup_request(const asn1::f1ap::ue_context_setup_request_s& msg)
{
  du_ue_index_t du_ue_index = INVALID_DU_UE_INDEX;

  if (msg->gnb_du_ue_f1ap_id_present) {
    const gnb_du_ue_f1ap_id_t gnb_du_ue_f1ap_id = int_to_gnb_du_ue_f1ap_id(msg->gnb_du_ue_f1ap_id);
    const f1ap_du_ue*         u                 = ues.find(gnb_du_ue_f1ap_id);
    if (u != nullptr) {
      du_ue_index = u->context.ue_index;
    }
  }
  if (du_ue_index == INVALID_DU_UE_INDEX) {
    // UE does not exist. Request free du ue index to schedule procedure.
    du_ue_index = du_mng.find_free_ue_index();
  }

  // Schedule UE Context Setup Procedure.
  du_mng.get_ue_handler(du_ue_index)
      .schedule_async_task(launch_async<f1ap_du_ue_context_setup_procedure>(msg, ues, du_mng, du_ue_index, ctxt));
}

void f1ap_du_impl::handle_ue_context_release_command(const asn1::f1ap::ue_context_release_cmd_s& msg)
{
  if (msg->old_gnb_du_ue_f1ap_id_present) {
    // If the old gNB-DU UE F1AP ID IE is included in the UE CONTEXT RELEASE COMMAND message, the gNB-DU shall
    // additionally release the UE context associated with the old gNB-DU UE F1AP ID.
    gnb_du_ue_f1ap_id_t old_gnb_du_ue_f1ap_id = int_to_gnb_du_ue_f1ap_id(msg->old_gnb_du_ue_f1ap_id);
    f1ap_du_ue*         old_ue                = ues.find(old_gnb_du_ue_f1ap_id);
    if (old_ue != nullptr) {
      du_mng.get_ue_handler(old_ue->context.ue_index)
          .schedule_async_task(du_mng.request_ue_removal(f1ap_ue_delete_request{old_ue->context.ue_index}));
    } else {
      logger.warning("old gNB-DU UE F1AP ID={} not found", fmt::underlying(old_gnb_du_ue_f1ap_id));
    }
  }

  gnb_du_ue_f1ap_id_t gnb_du_ue_f1ap_id = int_to_gnb_du_ue_f1ap_id(msg->gnb_du_ue_f1ap_id);
  f1ap_du_ue*         u                 = ues.find(gnb_du_ue_f1ap_id);
  if (u == nullptr) {
    logger.warning("Discarding UeContextReleaseCommand cause=Unrecognized gNB-DU UE F1AP ID={}",
                   fmt::underlying(gnb_du_ue_f1ap_id));
    // TODO: Handle.
    return;
  }

  du_mng.get_ue_handler(u->context.ue_index)
      .schedule_async_task(
          launch_async<f1ap_du_ue_context_release_procedure>(msg, ues, ctxt, timer_factory{timers, ctrl_exec}));
}

void f1ap_du_impl::handle_ue_context_modification_request(const asn1::f1ap::ue_context_mod_request_s& msg)
{
  gnb_du_ue_f1ap_id_t gnb_du_ue_f1ap_id = int_to_gnb_du_ue_f1ap_id(msg->gnb_du_ue_f1ap_id);
  f1ap_du_ue*         ue                = ues.find(gnb_du_ue_f1ap_id);

  if (ue == nullptr) {
    logger.error("Discarding UeContextModificationRequest cause=Unrecognized gNB-DU UE F1AP ID={}",
                 fmt::underlying(gnb_du_ue_f1ap_id));
    // TODO: Handle.
    return;
  }

  ue->handle_ue_context_modification_request(msg, ctxt);
}

void f1ap_du_impl::handle_dl_rrc_message_transfer(const asn1::f1ap::dl_rrc_msg_transfer_s& msg)
{
  gnb_du_ue_f1ap_id_t gnb_du_ue_f1ap_id = int_to_gnb_du_ue_f1ap_id(msg->gnb_du_ue_f1ap_id);

  // [TS38.473, 8.4.2.2.] If a UE-associated logical F1-connection exists, the DL RRC MESSAGE TRANSFER message shall
  // contain the gNB DU UE F1AP ID IE, which should be used by gNB-DU to lookup the stored UE context.
  f1ap_du_ue* ue = ues.find(gnb_du_ue_f1ap_id);

  if (ue == nullptr) {
    // [TS38.473, 8.4.2.2.] If no UE-associated logical F1-connection exists, the UE-associated logical F1-connection
    // shall be established at reception of the DL RRC MESSAGE TRANSFER message.
    logger.warning("du_ue={}: Discarding DLRRCMessageTransfer. Cause: No UE found with the provided gNB-DU-UE-F1AP-ID",
                   fmt::underlying(gnb_du_ue_f1ap_id));
    // TODO.
    return;
  }

  if (msg->old_gnb_du_ue_f1ap_id_present) {
    // [TS38.473, 8.4.2.2] The DL RRC MESSAGE TRANSFER message shall include, if available, the old gNB-DU UE F1AP ID
    // IE so that the gNB-DU can retrieve the existing UE context in RRC connection reestablishment procedure, as
    // defined in TS 38.401 [4].
    // [TS38.473, 8.4.2.2] If the gNB-DU identifies the UE-associated logical F1-connection by the gNB-DU UE F1AP ID IE
    // in the DL RRC MESSAGE TRANSFER message and the old gNB-DU UE F1AP ID IE is included, it shall release the old
    // gNB-DU UE F1AP ID and the related configurations associated with the old gNB-DU UE F1AP ID.
    const gnb_du_ue_f1ap_id_t old_gnb_du_ue_f1ap_id = int_to_gnb_du_ue_f1ap_id(msg->old_gnb_du_ue_f1ap_id);
    f1ap_du_ue*               old_ue                = ues.find(old_gnb_du_ue_f1ap_id);
    if (old_ue != nullptr) {
      // Delete gNB-CU-UE-F1AP-Id so that the UE is not reachable by the CU, and the gNB-CU-UE-F1AP-Id can be reused.
      old_ue->context.gnb_cu_ue_f1ap_id = gnb_cu_ue_f1ap_id_t::invalid;
      // Mark the old UE so that no UE context release procedure gets initiated.
      old_ue->context.marked_for_release = true;

      // Notify DU that the old UE needs to be released.
      du_mng.notify_reestablishment_of_old_ue(ue->context.ue_index, old_ue->context.ue_index);
    } else {
      logger.warning("old gNB-DU UE F1AP ID={} not found", fmt::underlying(old_gnb_du_ue_f1ap_id));
    }
  }

  // Handle gNB-CU UE F1AP ID.
  if (not handle_rx_message_gnb_cu_ue_f1ap_id(*ue, int_to_gnb_cu_ue_f1ap_id(msg->gnb_cu_ue_f1ap_id))) {
    return;
  }

  if (msg->new_gnb_cu_ue_f1ap_id_present) {
    // If the DL RRC MESSAGE TRANSFER message contains the New gNB-CU UE F1AP ID IE, the gNB-DU shall, if supported,
    // replace the value received in the gNB-CU UE F1AP ID IE by the value of the New gNB-CU UE F1AP ID and use it for
    // further signalling.
    ue->context.gnb_cu_ue_f1ap_id = int_to_gnb_cu_ue_f1ap_id(msg->new_gnb_cu_ue_f1ap_id);
  }

  if (ue->context.rrc_state == f1ap_ue_context::ue_rrc_state::no_config and not msg->old_gnb_du_ue_f1ap_id_present) {
    // If the UE has no dedicated configuration yet, we assume that this DL RRC Message Transfer contains it (e.g.
    // RRC Setup or RRC Reconfiguration). The only exception is when this DL RRC Message Transfer is an RRC
    // Reestablishment. In such case, we let the following DL RRC Message Transfer (with RRC Reconfiguration) to be the
    // one setting the state to config_pending.
    ue->context.rrc_state = f1ap_ue_context::ue_rrc_state::config_pending;
  }

  const srb_id_t srb_id     = int_to_srb_id(msg->srb_id);
  f1c_bearer*    srb_bearer = ue->bearers.find_srb(srb_id);
  if (srb_bearer == nullptr) {
    logger.warning("Discarding DLRRCMessageTransfer cause=SRB-ID={} not found", srb_id);
    cause_c cause;
    cause.set_radio_network().value = asn1::f1ap::cause_radio_network_opts::no_radio_res_available;
    send_error_indication(cause, std::nullopt, gnb_du_ue_f1ap_id, ue->context.gnb_cu_ue_f1ap_id);
    return;
  }

  // Forward SDU to lower layers.
  srb_bearer->handle_pdu(msg->rrc_container.copy(), msg->rrc_delivery_status_request_present);
}

std::string f1ap_du_impl::make_mbs_session_key(const asn1::f1ap::mbs_session_id_s& session_id)
{
  std::string key = session_id.tmgi.to_string();
  if (session_id.nid_present) {
    key += ":";
    key += session_id.nid.to_string();
  }
  return key;
}

uint64_t f1ap_du_impl::allocate_du_mbs_f1ap_id()
{
  return next_du_mbs_f1ap_id++;
}

std::string f1ap_du_impl::make_mbs_f1u_context_key(const std::string& multicast_f1u_context_ref, uint16_t mrb_id)
{
  return multicast_f1u_context_ref + ":" + std::to_string(mrb_id);
}

std::string f1ap_du_impl::get_mbs_f1u_local_addr() const
{
  // Co-located CU/DU builds use the local connector address space for F1-U. Split deployments can replace this once
  // the app-level F1-U bind address is threaded into F1AP MBS distribution setup.
  return "127.0.10.2";
}

static void append_unique_mrb_id(std::vector<uint16_t>& mrb_ids, uint16_t mrb_id)
{
  if (std::find(mrb_ids.begin(), mrb_ids.end(), mrb_id) == mrb_ids.end()) {
    mrb_ids.push_back(mrb_id);
  }
}

static pdcp_sn_size f1ap_pdcp_sn_len_to_pdcp_sn_size(const asn1::f1ap::pdcp_sn_len_e& sn_len)
{
  return sn_len.value == asn1::f1ap::pdcp_sn_len_opts::eighteen_bits ? pdcp_sn_size::size18bits
                                                                     : pdcp_sn_size::size12bits;
}

static void upsert_mrb_config(std::vector<f1ap_du_mbs_mrb_config>& configs, f1ap_du_mbs_mrb_config cfg)
{
  auto it = std::find_if(configs.begin(), configs.end(), [&cfg](const f1ap_du_mbs_mrb_config& item) {
    return item.mrb_id == cfg.mrb_id;
  });
  if (it == configs.end()) {
    configs.push_back(cfg);
  } else {
    *it = cfg;
  }
}

void f1ap_du_impl::handle_multicast_context_setup_request(const asn1::f1ap::multicast_context_setup_request_s& msg)
{
  const std::string key = make_mbs_session_key(msg->mbs_session_id);

  mbs_context& context = mbs_contexts[key];
  if (context.du_mbs_f1ap_id == 0) {
    context.du_mbs_f1ap_id = allocate_du_mbs_f1ap_id();
  }
  context.cu_mbs_f1ap_id = msg->gnb_cu_mbs_f1ap_id;
  context.session_id     = msg->mbs_session_id;
  context.mrb_ids.clear();
  context.mrb_configs.clear();

  f1ap_message resp_msg;
  resp_msg.pdu.set_successful_outcome().load_info_obj(ASN1_F1AP_ID_MULTICAST_CONTEXT_SETUP);
  auto& resp = resp_msg.pdu.successful_outcome().value.multicast_context_setup_resp();

  resp->gnb_cu_mbs_f1ap_id = context.cu_mbs_f1ap_id;
  resp->gnb_du_mbs_f1ap_id = context.du_mbs_f1ap_id;
  resp->multicast_m_rbs_setup_list.resize(msg->multicast_m_rbs_to_be_setup_list.size());

  for (uint32_t i = 0; i != msg->multicast_m_rbs_to_be_setup_list.size(); ++i) {
    const auto& req_item = msg->multicast_m_rbs_to_be_setup_list[i]->multicast_m_rbs_to_be_setup_item();
    resp->multicast_m_rbs_setup_list[i].load_info_obj(ASN1_F1AP_ID_MULTICAST_M_RBS_SETUP_ITEM);
    resp->multicast_m_rbs_setup_list[i]->multicast_m_rbs_setup_item().mrb_id = req_item.mrb_id;
  }

  f1ap_du_mbs_context_setup_request du_request;
  du_request.session_key        = key;
  du_request.gnb_cu_mbs_f1ap_id = context.cu_mbs_f1ap_id;
  du_request.gnb_du_mbs_f1ap_id = context.du_mbs_f1ap_id;
  du_request.mrb_ids.reserve(msg->multicast_m_rbs_to_be_setup_list.size());
  du_request.mrb_configs.reserve(msg->multicast_m_rbs_to_be_setup_list.size());
  for (const auto& mrb : msg->multicast_m_rbs_to_be_setup_list) {
    const auto&    req_item = mrb->multicast_m_rbs_to_be_setup_item();
    const uint16_t mrb_id   = req_item.mrb_id;
    f1ap_du_mbs_mrb_config cfg;
    cfg.mrb_id          = mrb_id;
    cfg.pdcp_sn_size_dl = f1ap_pdcp_sn_len_to_pdcp_sn_size(req_item.mbs_dl_pdcp_sn_len);
    append_unique_mrb_id(context.mrb_ids, mrb_id);
    upsert_mrb_config(context.mrb_configs, cfg);
    du_request.mrb_ids.push_back(mrb_id);
    du_request.mrb_configs.push_back(cfg);
  }
  du_mng.on_mbs_context_setup(du_request);

  logger.info("MBS session={}: Accepted multicast context setup cu_mbs_f1ap_id={} du_mbs_f1ap_id={} mrbs={}",
              key,
              context.cu_mbs_f1ap_id,
              context.du_mbs_f1ap_id,
              msg->multicast_m_rbs_to_be_setup_list.size());
  tx_pdu_notifier->on_new_message(resp_msg);
}

void f1ap_du_impl::handle_multicast_context_modification_request(
    const asn1::f1ap::multicast_context_mod_request_s& msg)
{
  auto context = std::find_if(mbs_contexts.begin(), mbs_contexts.end(), [&msg](const auto& entry) {
    return entry.second.cu_mbs_f1ap_id == msg->gnb_cu_mbs_f1ap_id and
           entry.second.du_mbs_f1ap_id == msg->gnb_du_mbs_f1ap_id;
  });

  if (context == mbs_contexts.end()) {
    f1ap_message fail_msg;
    fail_msg.pdu.set_unsuccessful_outcome().load_info_obj(ASN1_F1AP_ID_MULTICAST_CONTEXT_MOD);
    auto& fail                 = fail_msg.pdu.unsuccessful_outcome().value.multicast_context_mod_fail();
    fail->gnb_cu_mbs_f1ap_id   = msg->gnb_cu_mbs_f1ap_id;
    fail->gnb_du_mbs_f1ap_id   = msg->gnb_du_mbs_f1ap_id;
    fail->cause.set_radio_network().value =
        asn1::f1ap::cause_radio_network_opts::unknown_or_inconsistent_pair_of_mbs_f1ap_id;
    logger.warning(
        "cu_mbs_f1ap_id={} du_mbs_f1ap_id={}: Rejecting multicast context modification. Cause: context not found",
        msg->gnb_cu_mbs_f1ap_id,
        msg->gnb_du_mbs_f1ap_id);
    tx_pdu_notifier->on_new_message(fail_msg);
    return;
  }

  f1ap_message resp_msg;
  resp_msg.pdu.set_successful_outcome().load_info_obj(ASN1_F1AP_ID_MULTICAST_CONTEXT_MOD);
  auto& resp = resp_msg.pdu.successful_outcome().value.multicast_context_mod_resp();

  resp->gnb_cu_mbs_f1ap_id = context->second.cu_mbs_f1ap_id;
  resp->gnb_du_mbs_f1ap_id = context->second.du_mbs_f1ap_id;

  if (msg->multicast_m_rbs_to_be_setup_mod_list_present) {
    resp->multicast_m_rbs_setup_mod_list_present = true;
    resp->multicast_m_rbs_setup_mod_list.resize(msg->multicast_m_rbs_to_be_setup_mod_list.size());
    for (uint32_t i = 0; i != msg->multicast_m_rbs_to_be_setup_mod_list.size(); ++i) {
      const auto& req_item = msg->multicast_m_rbs_to_be_setup_mod_list[i]->multicast_m_rbs_to_be_setup_mod_item();
      append_unique_mrb_id(context->second.mrb_ids, req_item.mrb_id);
      f1ap_du_mbs_mrb_config cfg;
      cfg.mrb_id          = req_item.mrb_id;
      cfg.pdcp_sn_size_dl = f1ap_pdcp_sn_len_to_pdcp_sn_size(req_item.mbs_dl_pdcp_sn_len);
      upsert_mrb_config(context->second.mrb_configs, cfg);
      resp->multicast_m_rbs_setup_mod_list[i].load_info_obj(ASN1_F1AP_ID_MULTICAST_M_RBS_SETUP_MOD_ITEM);
      resp->multicast_m_rbs_setup_mod_list[i]->multicast_m_rbs_setup_mod_item().mrb_id = req_item.mrb_id;
    }
  }

  if (msg->multicast_m_rbs_to_be_modified_list_present) {
    resp->multicast_m_rbs_modified_list_present = true;
    resp->multicast_m_rbs_modified_list.resize(msg->multicast_m_rbs_to_be_modified_list.size());
    for (uint32_t i = 0; i != msg->multicast_m_rbs_to_be_modified_list.size(); ++i) {
      const auto& req_item = msg->multicast_m_rbs_to_be_modified_list[i]->multicast_m_rbs_to_be_modified_item();
      append_unique_mrb_id(context->second.mrb_ids, req_item.mrb_id);
      if (req_item.mbs_dl_pdcp_sn_len_present) {
        f1ap_du_mbs_mrb_config cfg;
        cfg.mrb_id          = req_item.mrb_id;
        cfg.pdcp_sn_size_dl = f1ap_pdcp_sn_len_to_pdcp_sn_size(req_item.mbs_dl_pdcp_sn_len);
        upsert_mrb_config(context->second.mrb_configs, cfg);
      }
      resp->multicast_m_rbs_modified_list[i].load_info_obj(ASN1_F1AP_ID_MULTICAST_M_RBS_MODIFIED_ITEM);
      resp->multicast_m_rbs_modified_list[i]->multicast_m_rbs_modified_item().mrb_id = req_item.mrb_id;
    }
  }

  if (msg->multicast_m_rbs_to_be_released_list_present) {
    for (const auto& item : msg->multicast_m_rbs_to_be_released_list) {
      const uint16_t mrb_id = item->multicast_m_rbs_to_be_released_item().mrb_id;
      context->second.mrb_ids.erase(std::remove(context->second.mrb_ids.begin(), context->second.mrb_ids.end(), mrb_id),
                                    context->second.mrb_ids.end());
      context->second.mrb_configs.erase(std::remove_if(context->second.mrb_configs.begin(),
                                                       context->second.mrb_configs.end(),
                                                       [mrb_id](const f1ap_du_mbs_mrb_config& cfg) {
                                                         return cfg.mrb_id == mrb_id;
                                                       }),
                                        context->second.mrb_configs.end());
    }
  }

  du_mng.on_mbs_context_modification({context->first,
                                      context->second.cu_mbs_f1ap_id,
                                      context->second.du_mbs_f1ap_id,
                                      context->second.mrb_ids,
                                      context->second.mrb_configs});

  logger.info("MBS session={}: Accepted multicast context modification cu_mbs_f1ap_id={} du_mbs_f1ap_id={} "
              "setup_mod_mrbs={} modified_mrbs={} active_mrbs={}",
              context->first,
              context->second.cu_mbs_f1ap_id,
              context->second.du_mbs_f1ap_id,
              msg->multicast_m_rbs_to_be_setup_mod_list.size(),
              msg->multicast_m_rbs_to_be_modified_list.size(),
              context->second.mrb_ids.size());
  tx_pdu_notifier->on_new_message(resp_msg);
}

void f1ap_du_impl::handle_multicast_context_release_command(const asn1::f1ap::multicast_context_release_cmd_s& msg)
{
  auto context = std::find_if(mbs_contexts.begin(), mbs_contexts.end(), [&msg](const auto& entry) {
    return entry.second.cu_mbs_f1ap_id == msg->gnb_cu_mbs_f1ap_id and
           entry.second.du_mbs_f1ap_id == msg->gnb_du_mbs_f1ap_id;
  });

  if (context == mbs_contexts.end()) {
    logger.warning("cu_mbs_f1ap_id={} du_mbs_f1ap_id={}: Ignoring multicast context release. Cause: context not found",
                   msg->gnb_cu_mbs_f1ap_id,
                   msg->gnb_du_mbs_f1ap_id);
    return;
  }

  f1ap_message resp_msg;
  resp_msg.pdu.set_successful_outcome().load_info_obj(ASN1_F1AP_ID_MULTICAST_CONTEXT_RELEASE);
  auto& resp = resp_msg.pdu.successful_outcome().value.multicast_context_release_complete();

  resp->gnb_cu_mbs_f1ap_id = context->second.cu_mbs_f1ap_id;
  resp->gnb_du_mbs_f1ap_id = context->second.du_mbs_f1ap_id;

  logger.info("MBS session={}: Released multicast context cu_mbs_f1ap_id={} du_mbs_f1ap_id={}",
              context->first,
              context->second.cu_mbs_f1ap_id,
              context->second.du_mbs_f1ap_id);
  du_mng.on_mbs_context_release(
      {context->first, context->second.cu_mbs_f1ap_id, context->second.du_mbs_f1ap_id});
  for (auto it = mbs_f1u_distribution_contexts.begin(); it != mbs_f1u_distribution_contexts.end();) {
    if (it->second.session_key == context->first) {
      const auto context_ref =
          it->first.substr(0, it->first.rfind(":"));
      auto ngu_receiver = mbs_ngu_receivers.find(it->first);
      if (ngu_receiver != mbs_ngu_receivers.end()) {
        ngu_receiver->second->stop();
        mbs_ngu_receivers.erase(ngu_receiver);
      } else {
        auto demux_session = mbs_f1u_demux_sessions.find(it->first);
        if (demux_session != mbs_f1u_demux_sessions.end()) {
          demux_session->second->stop();
          mbs_f1u_demux_sessions.erase(demux_session);
        } else {
          mbs_f1u_router->unbind_context({context_ref, it->second.mrb_id});
        }
      }
      it = mbs_f1u_distribution_contexts.erase(it);
    } else {
      ++it;
    }
  }
  mbs_contexts.erase(context);
  tx_pdu_notifier->on_new_message(resp_msg);
}

void f1ap_du_impl::handle_multicast_distribution_setup_request(
    const asn1::f1ap::multicast_distribution_setup_request_s& msg)
{
  const auto send_distribution_setup_failure = [this, &msg](cause_radio_network_opts::options cause) {
    f1ap_message fail_msg;
    fail_msg.pdu.set_unsuccessful_outcome().load_info_obj(ASN1_F1AP_ID_MULTICAST_DISTRIBUTION_SETUP);
    auto& fail = fail_msg.pdu.unsuccessful_outcome().value.multicast_distribution_setup_fail();
    fail->gnb_cu_mbs_f1ap_id                    = msg->gnb_cu_mbs_f1ap_id;
    fail->gnb_du_mbs_f1ap_id_present            = true;
    fail->gnb_du_mbs_f1ap_id                    = msg->gnb_du_mbs_f1ap_id;
    fail->mbs_multicast_f1_u_context_descriptor = msg->mbs_multicast_f1_u_context_descriptor;
    fail->cause.set_radio_network().value       = cause;
    tx_pdu_notifier->on_new_message(fail_msg);
  };

  auto context = std::find_if(mbs_contexts.begin(), mbs_contexts.end(), [&msg](const auto& entry) {
    return entry.second.cu_mbs_f1ap_id == msg->gnb_cu_mbs_f1ap_id and
           entry.second.du_mbs_f1ap_id == msg->gnb_du_mbs_f1ap_id;
  });

  if (context == mbs_contexts.end()) {
    logger.warning(
        "cu_mbs_f1ap_id={} du_mbs_f1ap_id={}: Ignoring multicast distribution setup. Cause: MBS context not found",
        msg->gnb_cu_mbs_f1ap_id,
        msg->gnb_du_mbs_f1ap_id);
    send_distribution_setup_failure(cause_radio_network_opts::unknown_or_inconsistent_pair_of_mbs_f1ap_id);
    return;
  }

  if (msg->multicast_f1_u_context_to_be_setup_list.size() == 0) {
    logger.warning("MBS session={}: Rejecting multicast distribution setup. Cause: empty MRB F1-U context list",
                   context->first);
    send_distribution_setup_failure(cause_radio_network_opts::unknown_or_inconsistent_mrb_id);
    return;
  }

  const std::string context_ref =
      msg->mbs_multicast_f1_u_context_descriptor.multicast_f1_u_context_ref_f1.to_string();

  for (const auto& item : msg->multicast_f1_u_context_to_be_setup_list) {
    const uint16_t mrb_id = item->multicast_f1_u_context_to_be_setup_item().mrb_id;
    if (std::find(context->second.mrb_ids.begin(), context->second.mrb_ids.end(), mrb_id) ==
        context->second.mrb_ids.end()) {
      logger.warning("MBS session={}: Rejecting multicast distribution setup. Cause: MRB {} is not configured",
                     context->first,
                     mrb_id);
      send_distribution_setup_failure(cause_radio_network_opts::unknown_or_inconsistent_mrb_id);
      return;
    }
  }

  f1ap_message resp_msg;
  resp_msg.pdu.set_successful_outcome().load_info_obj(ASN1_F1AP_ID_MULTICAST_DISTRIBUTION_SETUP);
  auto& resp = resp_msg.pdu.successful_outcome().value.multicast_distribution_setup_resp();

  resp->gnb_cu_mbs_f1ap_id                  = msg->gnb_cu_mbs_f1ap_id;
  resp->gnb_du_mbs_f1ap_id                  = msg->gnb_du_mbs_f1ap_id;
  resp->mbs_multicast_f1_u_context_descriptor = msg->mbs_multicast_f1_u_context_descriptor;
  resp->multicast_f1_u_context_ref_cu       = msg->mbs_multicast_f1_u_context_descriptor.multicast_f1_u_context_ref_f1;

  std::vector<std::string> bound_dist_keys;
  const auto cleanup_bound_contexts = [this, &bound_dist_keys, &context_ref]() {
    for (const std::string& dist_key : bound_dist_keys) {
      auto ngu_receiver = mbs_ngu_receivers.find(dist_key);
      if (ngu_receiver != mbs_ngu_receivers.end()) {
        ngu_receiver->second->stop();
        mbs_ngu_receivers.erase(ngu_receiver);
        mbs_f1u_distribution_contexts.erase(dist_key);
        continue;
      }
      auto demux_session = mbs_f1u_demux_sessions.find(dist_key);
      if (demux_session != mbs_f1u_demux_sessions.end()) {
        demux_session->second->stop();
        mbs_f1u_demux_sessions.erase(demux_session);
      } else {
        auto dist_context = mbs_f1u_distribution_contexts.find(dist_key);
        if (dist_context != mbs_f1u_distribution_contexts.end()) {
          mbs_f1u_router->unbind_context({context_ref, dist_context->second.mrb_id});
        }
      }
      mbs_f1u_distribution_contexts.erase(dist_key);
    }
  };

  // An IPv4 multicast traffic source (224.0.0.0/4) indicates an integrated gNB that ingests the MB-UPF NG-U shared
  // stream directly, rather than receiving the MRB over a unicast F1-U leg from a separate CU-UP.
  const auto is_ipv4_multicast = [](const up_transport_layer_info& tnl) {
    span<const uint8_t> bytes = tnl.tp_address.to_bytes();
    return bytes.size() == 4 and (bytes[0] & 0xf0U) == 0xe0U;
  };

  for (uint32_t i = 0; i != msg->multicast_f1_u_context_to_be_setup_list.size(); ++i) {
    const auto& req_item = msg->multicast_f1_u_context_to_be_setup_list[i]
                               ->multicast_f1_u_context_to_be_setup_item();
    const f1u_mbs_context_id router_id{context_ref, req_item.mrb_id};
    const up_transport_layer_info peer_tunnel = asn1_to_up_transport_layer_info(req_item.mbs_f1u_info_at_du);

    const auto data_handler = [this, session_key = context->first, mrb_id = req_item.mrb_id](nru_dl_message dl_msg) {
      f1ap_du_mbs_data_request request;
      request.session_key = session_key;
      request.mrb_id      = mrb_id;
      request.pdu         = std::move(dl_msg.t_pdu);
      du_mng.on_mbs_data_pdu(std::move(request));
    };

    const std::string dist_key = make_mbs_f1u_context_key(context_ref, req_item.mrb_id);
    if (is_ipv4_multicast(peer_tunnel)) {
      // Integrated gNB: join the MB-UPF NG-U shared multicast group and feed the decapsulated T-PDUs into the MTCH.
      if (mbs_f1u_exec == nullptr) {
        logger.warning("MBS session={}: Cannot set up NG-U multicast ingress ref={} mrb={}. Cause: no F1-U executor",
                       context->first,
                       context_ref,
                       req_item.mrb_id);
        cleanup_bound_contexts();
        send_distribution_setup_failure(cause_radio_network_opts::unspecified);
        return;
      }

      mbs_ngu_receiver_config ngu_cfg;
      ngu_cfg.multicast_group = peer_tunnel.tp_address.to_string();
      ngu_cfg.port            = GTPU_PORT;
      ngu_cfg.teid            = peer_tunnel.gtp_teid;

      auto tpdu_handler = [data_handler](byte_buffer t_pdu) {
        nru_dl_message dl_msg;
        dl_msg.t_pdu = std::move(t_pdu);
        data_handler(std::move(dl_msg));
      };

      auto receiver = std::make_unique<mbs_ngu_receiver>(std::move(ngu_cfg), *mbs_f1u_exec, std::move(tpdu_handler));
      if (not receiver->start()) {
        logger.warning("MBS session={}: Failed to start NG-U multicast ingress ref={} mrb={} group={}",
                       context->first,
                       context_ref,
                       req_item.mrb_id,
                       peer_tunnel.tp_address.to_string());
        cleanup_bound_contexts();
        send_distribution_setup_failure(cause_radio_network_opts::unspecified);
        return;
      }
      mbs_ngu_receivers[dist_key] = std::move(receiver);

      resp->multicast_f1_u_context_setup_list.push_back({});
      auto& resp_ie = resp->multicast_f1_u_context_setup_list.back();
      resp_ie.load_info_obj(ASN1_F1AP_ID_MULTICAST_F1_U_CONTEXT_SETUP_ITEM);
      auto& resp_item  = resp_ie->multicast_f1_u_context_setup_item();
      resp_item.mrb_id = req_item.mrb_id;
      // The DU ingests the shared multicast tunnel directly, so it echoes the same tunnel back as the CU endpoint.
      resp_item.mbs_f1u_info_at_cu = req_item.mbs_f1u_info_at_du;

      mbs_f1u_distribution_context dist_context;
      dist_context.session_key = context->first;
      dist_context.mrb_id      = req_item.mrb_id;
      dist_context.du_f1u_info = req_item.mbs_f1u_info_at_du;
      dist_context.cu_f1u_info = resp_item.mbs_f1u_info_at_cu;
      mbs_f1u_distribution_contexts[dist_key] = std::move(dist_context);
      bound_dist_keys.push_back(dist_key);
      continue;
    }
    if (mbs_demux_ctrl != nullptr and mbs_f1u_exec != nullptr) {
      auto session =
          std::make_unique<f1u_mbs_demux_session>(*mbs_f1u_router, *mbs_demux_ctrl, *mbs_f1u_exec, router_id);
      f1u_mbs_demux_bind_result bind_result =
          session->bind(get_mbs_f1u_local_addr(), peer_tunnel, data_handler);
      if (bind_result != f1u_mbs_demux_bind_result::success) {
        logger.warning("MBS session={}: Failed to bind multicast F1-U demux ref={} mrb={} result={}",
                       context->first,
                       context_ref,
                       req_item.mrb_id,
                       static_cast<unsigned>(bind_result));
        cleanup_bound_contexts();
        send_distribution_setup_failure(cause_radio_network_opts::unspecified);
        return;
      }
      mbs_f1u_demux_sessions[dist_key] = std::move(session);
    } else {
      f1u_mbs_bind_result bind_result =
          mbs_f1u_router->bind_context(router_id, get_mbs_f1u_local_addr(), peer_tunnel, data_handler);
      if (bind_result != f1u_mbs_bind_result::success) {
        logger.warning("MBS session={}: Failed to bind multicast F1-U ref={} mrb={} result={}",
                       context->first,
                       context_ref,
                       req_item.mrb_id,
                       static_cast<unsigned>(bind_result));
        cleanup_bound_contexts();
        send_distribution_setup_failure(cause_radio_network_opts::unspecified);
        return;
      }
    }

    const std::optional<f1u_mbs_tunnel_info> tunnel_info = mbs_f1u_router->get_tunnel_info(router_id);
    ocudu_assert(tunnel_info.has_value(), "MBS F1-U router returned success but no tunnel info");

    resp->multicast_f1_u_context_setup_list.push_back({});
    auto& resp_ie = resp->multicast_f1_u_context_setup_list.back();
    resp_ie.load_info_obj(ASN1_F1AP_ID_MULTICAST_F1_U_CONTEXT_SETUP_ITEM);
    auto& resp_item  = resp_ie->multicast_f1_u_context_setup_item();
    resp_item.mrb_id = req_item.mrb_id;
    up_transport_layer_info_to_asn1(resp_item.mbs_f1u_info_at_cu, tunnel_info->local);

    mbs_f1u_distribution_context dist_context;
    dist_context.session_key  = context->first;
    dist_context.mrb_id       = req_item.mrb_id;
    dist_context.du_f1u_info  = req_item.mbs_f1u_info_at_du;
    dist_context.cu_f1u_info  = resp_item.mbs_f1u_info_at_cu;
    mbs_f1u_distribution_contexts[dist_key] = std::move(dist_context);
    bound_dist_keys.push_back(dist_key);
  }

  logger.info("MBS session={}: Accepted multicast distribution setup ref={} mrbs={}",
              context->first,
              context_ref,
              msg->multicast_f1_u_context_to_be_setup_list.size());
  tx_pdu_notifier->on_new_message(resp_msg);
}

void f1ap_du_impl::handle_multicast_distribution_release_command(
    const asn1::f1ap::multicast_distribution_release_cmd_s& msg)
{
  auto context = std::find_if(mbs_contexts.begin(), mbs_contexts.end(), [&msg](const auto& entry) {
    return entry.second.cu_mbs_f1ap_id == msg->gnb_cu_mbs_f1ap_id and
           entry.second.du_mbs_f1ap_id == msg->gnb_du_mbs_f1ap_id;
  });

  if (context == mbs_contexts.end()) {
    logger.warning(
        "cu_mbs_f1ap_id={} du_mbs_f1ap_id={}: Ignoring multicast distribution release. Cause: MBS context not found",
        msg->gnb_cu_mbs_f1ap_id,
        msg->gnb_du_mbs_f1ap_id);
    return;
  }

  const std::string context_ref =
      msg->mbs_multicast_f1_u_context_descriptor.multicast_f1_u_context_ref_f1.to_string();
  const std::string key_prefix = context_ref + ":";
  unsigned          nof_erased = 0;
  for (auto it = mbs_f1u_distribution_contexts.begin(); it != mbs_f1u_distribution_contexts.end();) {
    if (it->first.rfind(key_prefix, 0) == 0 and it->second.session_key == context->first) {
      auto ngu_receiver = mbs_ngu_receivers.find(it->first);
      if (ngu_receiver != mbs_ngu_receivers.end()) {
        ngu_receiver->second->stop();
        mbs_ngu_receivers.erase(ngu_receiver);
      } else {
        auto demux_session = mbs_f1u_demux_sessions.find(it->first);
        if (demux_session != mbs_f1u_demux_sessions.end()) {
          demux_session->second->stop();
          mbs_f1u_demux_sessions.erase(demux_session);
        } else {
          mbs_f1u_router->unbind_context({context_ref, it->second.mrb_id});
        }
      }
      it = mbs_f1u_distribution_contexts.erase(it);
      ++nof_erased;
    } else {
      ++it;
    }
  }

  f1ap_message resp_msg;
  resp_msg.pdu.set_successful_outcome().load_info_obj(ASN1_F1AP_ID_MULTICAST_DISTRIBUTION_RELEASE);
  auto& resp = resp_msg.pdu.successful_outcome().value.multicast_distribution_release_complete();

  resp->gnb_cu_mbs_f1ap_id                    = msg->gnb_cu_mbs_f1ap_id;
  resp->gnb_du_mbs_f1ap_id                    = msg->gnb_du_mbs_f1ap_id;
  resp->mbs_multicast_f1_u_context_descriptor = msg->mbs_multicast_f1_u_context_descriptor;

  logger.info("MBS session={}: Released multicast distribution ref={} mrbs={}",
              context->first,
              context_ref,
              nof_erased);
  tx_pdu_notifier->on_new_message(resp_msg);
}

void f1ap_du_impl::handle_mbs_f1u_data(f1ap_du_mbs_f1u_data_indication indication)
{
  const std::string key = make_mbs_f1u_context_key(indication.multicast_f1u_context_ref, indication.mrb_id);
  auto              it  = mbs_f1u_distribution_contexts.find(key);
  if (it == mbs_f1u_distribution_contexts.end()) {
    logger.warning("Discarding multicast F1-U PDU ref={} mrb={}. Cause: distribution context not found",
                   indication.multicast_f1u_context_ref,
                   indication.mrb_id);
    return;
  }
  if (indication.nru.t_pdu.empty()) {
    logger.debug("Discarding multicast F1-U PDU ref={} mrb={}. Cause: empty T-PDU",
                 indication.multicast_f1u_context_ref,
                 indication.mrb_id);
    return;
  }

  if (not mbs_f1u_router->route_dl_pdu({indication.multicast_f1u_context_ref, indication.mrb_id},
                                       std::move(indication.nru))) {
    logger.warning("Discarding multicast F1-U PDU ref={} mrb={}. Cause: router binding not found",
                   indication.multicast_f1u_context_ref,
                   indication.mrb_id);
  }
}

void f1ap_du_impl::handle_ue_context_release_request(const f1ap_ue_context_release_request& request)
{
  f1ap_du_ue* ue = ues.find(request.ue_index);
  if (ue == nullptr) {
    logger.error("ue={}: Skipping UEContextReleaseRequest transmission. Cause: UE not found",
                 fmt::underlying(request.ue_index));
    return;
  }
  if (ue->context.gnb_cu_ue_f1ap_id == gnb_cu_ue_f1ap_id_t::invalid) {
    // The DU never received a F1AP PDU from the CU-CP assigning a gNB-CU-UE-F1AP-ID to the UE.
    logger.warning(
        "ue={} du_ue_id={}: Skipping UEContextReleaseRequest transmission. Cause: gNB-CU-UE-F1AP-ID does not exist",
        request.ue_index,
        fmt::underlying(ue->context.gnb_du_ue_f1ap_id));
    return;
  }

  if (ue->context.marked_for_release) {
    // UE context is already being released. Ignore the request.
    logger.debug("ue={} du_ue_id={}: Ignoring UEContextReleaseRequest. Cause: An UE Context Release procedure has "
                 "already started.",
                 request.ue_index,
                 fmt::underlying(ue->context.gnb_du_ue_f1ap_id));
    return;
  }

  f1ap_message msg;
  msg.pdu.set_init_msg().load_info_obj(ASN1_F1AP_ID_UE_CONTEXT_RELEASE_REQUEST);
  auto& rel_req = msg.pdu.init_msg().value.ue_context_release_request();

  rel_req->gnb_du_ue_f1ap_id = gnb_du_ue_f1ap_id_to_uint(ue->context.gnb_du_ue_f1ap_id);
  rel_req->gnb_cu_ue_f1ap_id = gnb_cu_ue_f1ap_id_to_uint(ue->context.gnb_cu_ue_f1ap_id);

  // Set F1AP cause.
  using cause_type = f1ap_ue_context_release_request::cause_type;
  if (request.cause == cause_type::rlf_mac or request.cause == cause_type::rlf_rlc) {
    rel_req->cause.set_radio_network().value =
        (request.cause == cause_type::rlf_rlc ? cause_radio_network_opts::rl_fail_rlc
                                              : cause_radio_network_opts::rl_fail_others);
  } else {
    rel_req->cause.set_radio_network().value = cause_radio_network_opts::cell_not_available;
  }

  ue->f1ap_msg_notifier.on_new_message(msg);
}

async_task<f1ap_ue_context_modification_confirm>
f1ap_du_impl::handle_ue_context_modification_required(const f1ap_ue_context_modification_required& msg)
{
  // TODO: add procedure implementation

  f1ap_event_manager::f1ap_ue_context_modification_outcome_t ue_ctxt_mod_resp;

  return launch_async([this, ue_ctxt_mod_resp, res = f1ap_ue_context_modification_confirm{}](
                          coro_context<async_task<f1ap_ue_context_modification_confirm>>& ctx) mutable {
    CORO_BEGIN(ctx);

    CORO_AWAIT_VALUE(ue_ctxt_mod_resp, events->f1ap_ue_context_modification_outcome);

    if (ue_ctxt_mod_resp.has_value()) {
      logger.debug("Received PDU with successful outcome");
      res.success = true;
    } else {
      logger.debug("Received PDU with unsuccessful outcome");
      res.success = false;
    }

    CORO_RETURN(res);
  });
}

void f1ap_du_impl::handle_message(const f1ap_message& msg)
{
  using pdu_types = f1ap_pdu_c::types_opts;

  // Run F1AP protocols in Control executor.
  if (not ctrl_exec.execute([this, msg]() {
        // Log message.
        log_pdu(true, msg);

        // Record metric.
        metrics.on_rx_pdu(msg);

        switch (msg.pdu.type().value) {
          case pdu_types::init_msg:
            handle_initiating_message(msg.pdu.init_msg());
            break;
          case pdu_types::successful_outcome:
            handle_successful_outcome(msg.pdu.successful_outcome());
            break;
          case pdu_types::unsuccessful_outcome:
            handle_unsuccessful_outcome(msg.pdu.unsuccessful_outcome());
            break;
          default:
            logger.error("Invalid PDU type");
            break;
        }
      })) {
    logger.error("Unable to dispatch handling of F1AP PDU. Cause: DU task queue is full");
    // TODO: Handle.
    return;
  }
}

void f1ap_du_impl::handle_initiating_message(const init_msg_s& msg)
{
  using msg_types = f1ap_elem_procs_o::init_msg_c::types_opts;

  switch (msg.value.type().value) {
    case msg_types::reset:
      handle_reset(msg.value.reset());
      break;
    case msg_types::gnb_cu_cfg_upd:
      handle_gnb_cu_configuration_update(msg.value.gnb_cu_cfg_upd());
      break;
    case msg_types::dl_rrc_msg_transfer:
      handle_dl_rrc_message_transfer(msg.value.dl_rrc_msg_transfer());
      break;
    case msg_types::ue_context_setup_request:
      handle_ue_context_setup_request(msg.value.ue_context_setup_request());
      break;
    case msg_types::ue_context_mod_request:
      handle_ue_context_modification_request(msg.value.ue_context_mod_request());
      break;
    case msg_types::ue_context_release_cmd:
      handle_ue_context_release_command(msg.value.ue_context_release_cmd());
      break;
    case msg_types::paging:
      handle_paging_request(msg.value.paging());
      break;
    case msg_types::multicast_context_setup_request:
      handle_multicast_context_setup_request(msg.value.multicast_context_setup_request());
      break;
    case msg_types::multicast_context_mod_request:
      handle_multicast_context_modification_request(msg.value.multicast_context_mod_request());
      break;
    case msg_types::multicast_context_release_cmd:
      handle_multicast_context_release_command(msg.value.multicast_context_release_cmd());
      break;
    case msg_types::multicast_distribution_setup_request:
      handle_multicast_distribution_setup_request(msg.value.multicast_distribution_setup_request());
      break;
    case msg_types::multicast_distribution_release_cmd:
      handle_multicast_distribution_release_command(msg.value.multicast_distribution_release_cmd());
      break;
    case msg_types::positioning_meas_request:
      handle_positioning_measurement_request(msg.value.positioning_meas_request());
      break;
    case msg_types::trp_info_request:
      handle_trp_information_request(msg.value.trp_info_request());
      break;
    case msg_types::positioning_info_request:
      handle_positioning_information_request(msg.value.positioning_info_request());
      break;
    default:
      logger.error("Initiating message of type {} is not supported", msg.value.type().to_string());
  }
}

void f1ap_du_impl::handle_successful_outcome(const asn1::f1ap::successful_outcome_s& outcome)
{
  std::optional<uint8_t> transaction_id = get_transaction_id(outcome);
  if (not transaction_id.has_value()) {
    logger.error("Successful outcome of type {} is not supported", outcome.value.type().to_string());
    return;
  }

  // Set transaction result and resume suspended procedure.
  if (not events->transactions.set_response(transaction_id.value(), outcome)) {
    logger.warning("Unexpected transaction id={}", transaction_id.value());
  }
}

void f1ap_du_impl::handle_unsuccessful_outcome(const asn1::f1ap::unsuccessful_outcome_s& outcome)
{
  std::optional<uint8_t> transaction_id = get_transaction_id(outcome);
  if (not transaction_id.has_value()) {
    logger.error("Unsuccessful outcome of type {} is not supported", outcome.value.type().to_string());
    return;
  }

  // Set transaction result and resume suspended procedure.
  if (not events->transactions.set_response(transaction_id.value(), make_unexpected(outcome))) {
    logger.warning("Unexpected transaction id={}", transaction_id.value());
  }
}

bool f1ap_du_impl::handle_rx_message_gnb_cu_ue_f1ap_id(f1ap_du_ue& ue, gnb_cu_ue_f1ap_id_t gnb_cu_ue_f1ap_id)
{
  if (ue.context.gnb_cu_ue_f1ap_id == gnb_cu_ue_f1ap_id) {
    // No update in gNB-CU UE F1AP ID.
    return true;
  }

  const f1ap_du_ue* ue_cu_id = ues.find(gnb_cu_ue_f1ap_id);
  if (ue.context.gnb_cu_ue_f1ap_id == gnb_cu_ue_f1ap_id_t::invalid and ue_cu_id == nullptr) {
    // First message from the gNB-CU for this UE. Update gNB-CU-UE-F1AP-ID.
    ue.context.gnb_cu_ue_f1ap_id = gnb_cu_ue_f1ap_id;
    return true;
  }
  cause_c cause;
  if (ue_cu_id == nullptr) {
    cause.set_radio_network().value = cause_radio_network_opts::unknown_or_inconsistent_pair_of_ue_f1ap_id;
    logger.warning("Discarding message. Cause: gNB-CU UE F1AP ID={} does not match existing context",
                   fmt::underlying(gnb_cu_ue_f1ap_id));
  } else {
    // [TS38.473, Cause IE] The action failed because the gNB-CU UE F1AP ID is either unknown, or (for a first
    // message received at the gNB-CU) is known and already allocated to an existing context.
    cause.set_radio_network().value = cause_radio_network_opts::unknown_or_already_allocated_gnb_cu_ue_f1ap_id;
    logger.warning("Discarding message cause=gNB-CU UE F1AP ID={} is known and already allocated to an existing "
                   "context with gNB-DU UE F1AP ID={}",
                   fmt::underlying(gnb_cu_ue_f1ap_id),
                   fmt::underlying(ue_cu_id->context.gnb_du_ue_f1ap_id));
  }
  send_error_indication(cause, std::nullopt, ue.context.gnb_du_ue_f1ap_id, ue.context.gnb_cu_ue_f1ap_id);
  return false;
}

void f1ap_du_impl::send_error_indication(const asn1::f1ap::cause_c&         cause,
                                         std::optional<uint8_t>             transaction_id,
                                         std::optional<gnb_du_ue_f1ap_id_t> du_ue_id,
                                         std::optional<gnb_cu_ue_f1ap_id_t> cu_ue_id)
{
  f1ap_message msg;
  msg.pdu.set_init_msg().load_info_obj(ASN1_F1AP_ID_ERROR_IND);
  auto& err_ind = msg.pdu.init_msg().value.error_ind();

  if (transaction_id.has_value()) {
    err_ind->transaction_id = transaction_id.value();
  } else {
    auto transaction        = events->transactions.create_transaction();
    err_ind->transaction_id = transaction.id();
  }

  // Set cause.
  err_ind->cause_present = true;
  err_ind->cause         = cause;

  // [TS 38.473, 8.2.2.2] In case the Error Indication procedure is triggered by utilising UE associated signalling
  // the gNB-CU UE F1AP ID IE and gNBDU UE F1AP ID IE shall be included in the ERROR INDICATION message.
  err_ind->gnb_cu_ue_f1ap_id_present = cu_ue_id.has_value();
  if (err_ind->gnb_cu_ue_f1ap_id_present) {
    err_ind->gnb_cu_ue_f1ap_id = gnb_cu_ue_f1ap_id_to_uint(*cu_ue_id);
  }
  err_ind->gnb_du_ue_f1ap_id_present = du_ue_id.has_value();
  if (err_ind->gnb_du_ue_f1ap_id_present) {
    err_ind->gnb_du_ue_f1ap_id = gnb_du_ue_f1ap_id_to_uint(*du_ue_id);
  }

  // Send message to CU.
  tx_pdu_notifier->on_new_message(msg);
}

void f1ap_du_impl::handle_paging_request(const asn1::f1ap::paging_s& msg)
{
  if (not odu::handle_paging_request(msg, paging_notifier, ctxt, logger)) {
    // Failed to handle paging request. Send error indication.
    asn1::f1ap::cause_c cause;
    cause.set_misc().value = asn1::f1ap::cause_misc_opts::unspecified;
    send_error_indication(cause, std::nullopt, std::nullopt, std::nullopt);
  }
}

void f1ap_du_impl::handle_positioning_measurement_request(const positioning_meas_request_s& msg)
{
  du_mng.schedule_async_task(
      launch_async<f1ap_du_positioning_measurement_procedure>(msg, du_mng.get_positioning_handler(), *tx_pdu_notifier));
}

void f1ap_du_impl::handle_trp_information_request(const trp_info_request_s& msg)
{
  du_mng.schedule_async_task(launch_async<f1ap_du_trp_information_exchange_procedure>(
      msg, du_mng.get_positioning_handler(), *tx_pdu_notifier));
}

void f1ap_du_impl::handle_positioning_information_request(const asn1::f1ap::positioning_info_request_s& msg)
{
  gnb_du_ue_f1ap_id_t gnb_du_ue_f1ap_id = int_to_gnb_du_ue_f1ap_id(msg->gnb_du_ue_f1ap_id);
  f1ap_du_ue*         ue                = ues.find(gnb_du_ue_f1ap_id);

  if (ue == nullptr) {
    // UE not found.
    // TODO: Handle
    return;
  }

  du_mng.get_ue_handler(ue->context.ue_index)
      .schedule_async_task(
          launch_async<f1ap_du_positioning_information_exchange_procedure>(msg, du_mng.get_positioning_handler(), *ue));
}

std::optional<gnb_cu_ue_f1ap_id_t> f1ap_du_impl::get_gnb_cu_ue_f1ap_id(const du_ue_index_t& ue_index) const
{
  const f1ap_du_ue* ue = ues.find(ue_index);
  if (ue == nullptr or ue->context.gnb_cu_ue_f1ap_id == gnb_cu_ue_f1ap_id_t::invalid) {
    return std::nullopt;
  }
  return ue->context.gnb_cu_ue_f1ap_id;
}

std::optional<gnb_cu_ue_f1ap_id_t>
f1ap_du_impl::get_gnb_cu_ue_f1ap_id(const gnb_du_ue_f1ap_id_t& gnb_du_ue_f1ap_id) const
{
  const f1ap_du_ue* ue = ues.find(gnb_du_ue_f1ap_id);
  if (ue == nullptr or ue->context.gnb_cu_ue_f1ap_id == gnb_cu_ue_f1ap_id_t::invalid) {
    return std::nullopt;
  }
  return ue->context.gnb_cu_ue_f1ap_id;
}

gnb_du_ue_f1ap_id_t f1ap_du_impl::get_gnb_du_ue_f1ap_id(const du_ue_index_t& ue_index)
{
  gnb_du_ue_f1ap_id_t gnb_du_ue_f1ap_id = gnb_du_ue_f1ap_id_t::invalid;
  const f1ap_du_ue*   ue                = ues.find(ue_index);
  if (ue) {
    gnb_du_ue_f1ap_id = ue->context.gnb_du_ue_f1ap_id;
  }
  return gnb_du_ue_f1ap_id;
}
gnb_du_ue_f1ap_id_t f1ap_du_impl::get_gnb_du_ue_f1ap_id(const gnb_cu_ue_f1ap_id_t& gnb_cu_ue_f1ap_id)
{
  gnb_du_ue_f1ap_id_t gnb_du_ue_f1ap_id = gnb_du_ue_f1ap_id_t::invalid;
  const f1ap_du_ue*   ue                = ues.find(gnb_cu_ue_f1ap_id);
  if (ue) {
    gnb_du_ue_f1ap_id = ue->context.gnb_du_ue_f1ap_id;
  }
  return gnb_du_ue_f1ap_id;
}

du_ue_index_t f1ap_du_impl::get_ue_index(const gnb_du_ue_f1ap_id_t& gnb_du_ue_f1ap_id)
{
  du_ue_index_t     du_ue_index = du_ue_index_t::INVALID_DU_UE_INDEX;
  const f1ap_du_ue* ue          = ues.find(gnb_du_ue_f1ap_id);
  if (ue) {
    du_ue_index = ue->context.ue_index;
  }
  return du_ue_index;
}

du_ue_index_t f1ap_du_impl::get_ue_index(const gnb_cu_ue_f1ap_id_t& gnb_cu_ue_f1ap_id)
{
  du_ue_index_t     du_ue_index = du_ue_index_t::INVALID_DU_UE_INDEX;
  const f1ap_du_ue* ue          = ues.find(gnb_cu_ue_f1ap_id);
  if (ue) {
    du_ue_index = ue->context.ue_index;
  }
  return du_ue_index;
}

void f1ap_du_impl::handle_access_success(const f1ap_access_success_event& msg)
{
  f1ap_du_ue* ue = ues.find(msg.ue_index);
  if (ue == nullptr) {
    logger.error("ue={}: Skipping AccessSuccess transmission. Cause: UE not found", fmt::underlying(msg.ue_index));
    return;
  }
  if (ue->context.gnb_cu_ue_f1ap_id == gnb_cu_ue_f1ap_id_t::invalid) {
    logger.warning("ue={} du_ue_id={}: Skipping AccessSuccess transmission. Cause: gNB-CU-UE-F1AP-ID does not exist",
                   msg.ue_index,
                   fmt::underlying(ue->context.gnb_du_ue_f1ap_id));
    return;
  }

  f1ap_message f1ap_msg;
  f1ap_msg.pdu.set_init_msg().load_info_obj(ASN1_F1AP_ID_ACCESS_SUCCESS);
  auto& access_succ = f1ap_msg.pdu.init_msg().value.access_success();

  access_succ->gnb_du_ue_f1ap_id = gnb_du_ue_f1ap_id_to_uint(ue->context.gnb_du_ue_f1ap_id);
  access_succ->gnb_cu_ue_f1ap_id = gnb_cu_ue_f1ap_id_to_uint(ue->context.gnb_cu_ue_f1ap_id);
  access_succ->nr_cgi            = cgi_to_asn1(msg.cgi);

  ue->f1ap_msg_notifier.on_new_message(f1ap_msg);
}

void f1ap_du_impl::log_pdu(bool is_rx, const f1ap_message& msg)
{
  using namespace asn1::f1ap;

  if (not logger.info.enabled()) {
    return;
  }

  // Fetch UE index.
  auto                         cu_ue_id = ocudu::get_gnb_du_ue_f1ap_id(msg.pdu);
  std::optional<du_ue_index_t> ue_idx;
  if (cu_ue_id.has_value()) {
    auto* ue_ptr = ues.find(cu_ue_id.value());
    if (ue_ptr != nullptr and ue_ptr->context.ue_index != INVALID_DU_UE_INDEX) {
      ue_idx = ue_ptr->context.ue_index;
    }
  }

  // Log PDU.
  log_f1ap_pdu(logger, is_rx, ctxt.du_id, ue_idx, msg, logger.debug.enabled());
}
