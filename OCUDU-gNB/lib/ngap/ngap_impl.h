...
private:
  ...
  /// \brief Notify about the reception of an MBS Multicast Session Activation Request.
  void handle_multicast_session_activation_request(const asn1::ngap::multicast_session_activation_request_s& msg);

  /// \brief Notify about the reception of an MBS Multicast Session Deactivation Request.
  void handle_multicast_session_deactivation_request(const asn1::ngap::multicast_session_deactivation_request_s& msg);

  /// \brief Notify about the reception of an MBS Multicast Session Update Request.
  void handle_multicast_session_update_request(const asn1::ngap::multicast_session_upd_request_s& msg);

  /// \brief Notify about the reception of an MBS Multicast Group Paging message.
  void handle_multicast_group_paging(const asn1::ngap::multicast_group_paging_s& msg);

  /// \brief Notify about the reception of an MBS Broadcast Session Setup Request, as per TS 38.413, Section 8.13.1.
  void handle_broadcast_session_setup_request(const asn1::ngap::broadcast_session_setup_request_s& msg);

  /// \brief Notify about the reception of an MBS Broadcast Session Release Request, as per TS 38.413, Section 8.13.3.
  void handle_broadcast_session_release_request(const asn1::ngap::broadcast_session_release_request_s& msg);
  ...
  struct mbs_session_context {
    asn1::ngap::mbs_session_id_s id;
    bool                         active = false;
    std::optional<uint32_t>      area_session_id;
    bool                         has_service_area    = false;
    bool                         has_qos_flows       = false;
    bool                         has_ng_u_tnl_info   = false;
    bool                         has_released_flows  = false;
  };

  std::string make_mbs_session_key(const asn1::ngap::mbs_session_id_s& session_id) const;
  void        send_multicast_session_activation_response(const asn1::ngap::mbs_session_id_s& session_id);
  void        send_multicast_session_activation_failure(const asn1::ngap::mbs_session_id_s& session_id,
                                                       asn1::ngap::cause_radio_network_opts::options cause);
  void        send_multicast_session_deactivation_response(const asn1::ngap::mbs_session_id_s& session_id);
  void        send_multicast_session_update_response(const asn1::ngap::mbs_session_id_s& session_id,
                                                     std::optional<uint32_t> area_session_id);
  void        send_multicast_session_update_failure(const asn1::ngap::mbs_session_id_s& session_id,
                                                    std::optional<uint32_t> area_session_id,
                                                    asn1::ngap::cause_radio_network_opts::options cause);
  void        send_broadcast_session_setup_response(const asn1::ngap::mbs_session_id_s& session_id);
  void        send_broadcast_session_setup_failure(const asn1::ngap::mbs_session_id_s& session_id,
                                                   asn1::ngap::cause_radio_network_opts::options cause);
  void        send_broadcast_session_release_response(const asn1::ngap::mbs_session_id_s& session_id);
  ...
  /// Repository of NGAP MBS session contexts keyed by TMGI/NID.
  std::unordered_map<std::string, mbs_session_context> mbs_sessions;
