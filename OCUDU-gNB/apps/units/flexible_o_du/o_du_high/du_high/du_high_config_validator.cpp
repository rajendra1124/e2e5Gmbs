...
static bool validate_cell_sib_config(const du_high_unit_base_cell_config& cell_cfg)
{
  ...
  for (const auto& si_msg : sib_cfg.si_sched_info) {
    ...
    // Release-17 SI entries are encoded via SchedulingInfo2 and cannot be mixed with other SIBs in this packer.
    const bool has_sib19 = std::find(si_msg.sib_mapping_info.begin(),
                                     si_msg.sib_mapping_info.end(),
                                     static_cast<std::underlying_type_t<sib_type>>(sib_type::sib19)) !=
                           si_msg.sib_mapping_info.end();
    const bool has_sib20 = std::find(si_msg.sib_mapping_info.begin(),
                                     si_msg.sib_mapping_info.end(),
                                     static_cast<std::underlying_type_t<sib_type>>(sib_type::sib20)) !=
                           si_msg.sib_mapping_info.end();
    if ((has_sib19 or has_sib20) && si_msg.sib_mapping_info.size() > 1) {
      fmt::print("SIB19/SIB20 cannot be included in the SI messages together with other SIBs.\n");
      return false;
    }
  }
