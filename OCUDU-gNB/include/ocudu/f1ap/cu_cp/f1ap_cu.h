...
struct f1ap_mbs_session_update_request {
  cu_cp_mbs_session_context session;
};

struct f1ap_mbs_session_release_request {
  cu_cp_mbs_session_id session_id;
};

/// Handle F1AP MBS context procedures as defined in TS 38.473 section 8.
class f1ap_mbs_manager
{
public:
  virtual ~f1ap_mbs_manager() = default;

  /// \brief Initiates multicast context setup or modification for an MBS session.
  virtual void handle_mbs_session_update_request(const f1ap_mbs_session_update_request& request) = 0;

  /// \brief Initiates multicast context release for an MBS session.
  virtual void handle_mbs_session_release_request(const f1ap_mbs_session_release_request& request) = 0;
};
...
public:
  ...
  virtual f1ap_mbs_manager&                  get_f1ap_mbs_manager()                  = 0;
