...
struct f1ap_du_mbs_mrb_config {
  uint16_t     mrb_id          = 0;
  pdcp_sn_size pdcp_sn_size_dl = pdcp_sn_size::size12bits;
};

struct f1ap_du_mbs_context_setup_request {
...
struct f1ap_du_mbs_context_modification_request {
  std::string           session_key;
  uint64_t              gnb_cu_mbs_f1ap_id = 0;
  uint64_t              gnb_du_mbs_f1ap_id = 0;
  std::vector<uint16_t>                mrb_ids;
...
struct f1ap_du_mbs_context_release_request {
  std::string session_key;
  uint64_t    gnb_cu_mbs_f1ap_id = 0;
  uint64_t    gnb_du_mbs_f1ap_id = 0;
};

struct f1ap_du_mbs_data_request {
  std::string session_key;
  uint16_t    mrb_id = 0;
  byte_buffer pdu;
};

struct f1ap_du_mbs_f1u_data_indication {
  std::string    multicast_f1u_context_ref;
  uint16_t       mrb_id = 0;
  nru_dl_message nru;
};

class f1ap_du_mbs_f1u_data_handler
{
public:
  virtual ~f1ap_du_mbs_f1u_data_handler() = default;

  /// Deliver one multicast F1-U/NR-U DL payload identified by the F1-U distribution context.
  virtual void handle_mbs_f1u_data(f1ap_du_mbs_f1u_data_indication indication) = 0;
};
...
public:
  ...
  /// Store DU-wide MBS context created from F1AP multicast context setup.
  virtual void on_mbs_context_setup(const f1ap_du_mbs_context_setup_request& request) = 0;

  /// Modify DU-wide MBS context from F1AP multicast context modification.
  virtual void on_mbs_context_modification(const f1ap_du_mbs_context_modification_request& request) = 0;

  /// Remove DU-wide MBS context released from F1AP multicast context release.
  virtual void on_mbs_context_release(const f1ap_du_mbs_context_release_request& request) = 0;

  /// Deliver one multicast F1-U/NR-U payload into the DU MTCH path.
  virtual void on_mbs_data_pdu(f1ap_du_mbs_data_request request) = 0;
...
class f1ap_du : public f1ap_message_handler,
                ...
                public f1ap_du_mbs_f1u_data_handler
{
public:
