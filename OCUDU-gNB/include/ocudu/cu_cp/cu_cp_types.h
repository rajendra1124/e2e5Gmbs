...
struct cu_cp_mbs_session_id {
  std::string                tmgi;
  std::optional<std::string> nid;
};

struct cu_cp_mbs_session_update_request {
  cu_cp_mbs_session_id   session_id;
  std::optional<uint32_t> area_session_id;
  std::vector<uint8_t>    qos_flow_ids;
  bool                   service_area_present        = false;
  bool                   qos_flows_present           = false;
  bool                   ngu_tnl_info_present        = false;
  bool                   qos_flow_release_present    = false;
  std::optional<up_transport_layer_info> ngu_tnl_info;
};

struct cu_cp_mbs_session_activation_request {
  cu_cp_mbs_session_id session_id;
};

struct cu_cp_mbs_session_deactivation_request {
  cu_cp_mbs_session_id session_id;
};

struct cu_cp_mbs_group_paging_message {
  cu_cp_mbs_session_id session_id;
  unsigned             nof_paging_area_entries = 0;
};
